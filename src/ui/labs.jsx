...
const categories = ["All tests", "Blood", "Urine", "Scan", "Culture"];

export default function LabsPage() {
  useEffect(() => {
    document.title = "Lab tests & home collection — MedConnect";
  }, []);
  const [selectedLab, setSelectedLab] = useState(labs[0].id);
  const [cat, setCat] = useState("All tests");
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-8 grid lg:grid-cols-[260px_1fr] gap-6">
        <aside className="space-y-4">
          <div className="rounded-xl bg-card border border-border p-4">
            ...
            <div className="space-y-2">
              {labs.map((l) => (
                <button key={l.id} onClick={() => setSelectedLab(l.id)} className={`w-full text-left rounded-lg p-3 border transition ${selectedLab === l.id ? "bg-primary-soft border-primary text-primary" : "bg-card border-border hover:border-primary/40"}`}>
                  <div className="text-sm font-semibold">{l.name}</div>
                  <div className="text-[11px] mt-0.5 opacity-70 flex items-center gap-1"><Star className="h-3 w-3 fill-current" /> {l.rating} · {l.address.split(",")[0]}</div>
                </button>
              ))}
            </div>
          </div>
        ...
        <main>
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            {categories.map((c) => (
              <button key={c} onClick={() => setCat(c)} className={`rounded-full px-3.5 py-1.5 text-xs font-medium border transition ${cat === c ? "bg-primary text-primary-foreground border-primary" : "bg-card text-muted-foreground border-border hover:text-foreground hover:border-primary/40"}`}>{c}</button>
            ))}
          </div>
                ...
                <div key={t.id} className="rounded-xl bg-card border border-border p-4 hover:border-primary/40 transition">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">{t.sample} · Report in {t.reportHrs} hrs · {t.fasting}</p>
          ...
          <div className="max-w-[1200px] mx-auto px-6 py-4">
            <div className="flex flex-wrap items-center gap-4 justify-between">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-full bg-success-soft text-success flex items-center justify-center"><ShoppingCart className="h-5 w-5" /></div>
                <div>
              ...
              <div className="inline-flex rounded-lg bg-secondary p-1 text-xs">
                <button onClick={() => setMode("home")} className={`px-3 py-1.5 rounded-md font-medium transition ${mode === "home" ? "bg-card shadow-sm text-foreground" : "text-muted-foreground"}`}><Home className="h-3.5 w-3.5 inline mr-1" /> Home collection</button>
                <button onClick={() => setMode("walkin")} className={`px-3 py-1.5 rounded-md font-medium transition ${mode === "walkin" ? "bg-card shadow-sm text-foreground" : "text-muted-foreground"}`}><Building2 className="h-3.5 w-3.5 inline mr-1" /> Walk-in</button>
              </div>
              ...
              <button onClick={() => setBooked(true)} className="inline-flex items-center gap-2 rounded-lg bg-success text-success-foreground px-5 py-3 text-sm font-semibold hover:bg-success-dark transition">
                {mode === "home" ? "Schedule pickup" : "Book slot"}<Clock className="h-4 w-4" />
              </button>
            </div>
          </div>
        </div>
      ...
      {booked && (
        <div className="fixed inset-0 z-50 bg-foreground/40 backdrop-blur flex items-center justify-center p-4">
          <div className="w-full max-w-md rounded-2xl bg-card border border-border shadow-elevated p-7 text-center">
            <div className="mx-auto h-14 w-14 rounded-full bg-success text-success-foreground flex items-center justify-center mb-4"><Check className="h-7 w-7" /></div>
            <h3 className="text-xl font-bold">Booking confirmed</h3>
            <p className="text-sm text-muted-foreground mt-1">{mode === "home" ? "Phlebotomist will arrive at your address in the selected slot." : "Walk in any time within your slot — no waiting."}</p>
