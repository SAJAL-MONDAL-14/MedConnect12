...
const SORTS = ["Distance", "Rating", "Bed availability", "Drive time"];
...
export default function SearchPage() {
  ...
  const [chips, setChips] = useState(
    searchFilters ? searchFilters.split(",").filter(Boolean) : ["Specialty: Cardiology", "Distance: 5km", "Has beds", "Rating 4+"],
  );

  useEffect(() => {
    document.title = "Search hospitals & doctors — MedConnect";
  }, []);
  ...
  const results = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      list = list.filter((h) => h.name.toLowerCase().includes(q) || h.city.toLowerCase().includes(q) || h.specialties.some((s) => s.toLowerCase().includes(q)));
    }
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {chips.map((f) => (
                <span key={f} className="inline-flex items-center gap-1.5 rounded-full bg-primary-soft text-primary px-2.5 py-1 text-xs font-medium">
                  {f}<button onClick={() => setChips((c) => c.filter((x) => x !== f))} className="hover:text-primary-dark"><X className="h-3 w-3" /></button>
                </span>
              ))}
              ...
              <div className="relative">
                ...
                {sortOpen && (
                  <div className="absolute right-0 top-full mt-1 w-44 rounded-md bg-card border border-border shadow-elevated z-20 py-1">
                    {SORTS.map((s) => (
                      <button key={s} onClick={() => { setSort(s); setSortOpen(false); }} className={`w-full text-left px-3 py-1.5 text-sm hover:bg-secondary ${sort === s ? "text-primary font-medium" : "text-foreground"}`}>{s}</button>
                    ))}
                  </div>
                )}
              </div>
              <div className="inline-flex rounded-md border border-border bg-card overflow-hidden text-xs">
                <button onClick={() => setView("list")} className={`inline-flex items-center gap-1.5 px-3 py-1.5 transition ${view === "list" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted"}`}><List className="h-3.5 w-3.5" /> List</button>
                <button onClick={() => setView("map")} className={`inline-flex items-center gap-1.5 px-3 py-1.5 transition ${view === "map" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted"}`}><MapIcon className="h-3.5 w-3.5" /> Map</button>
              </div>
            </div>
          </div>
          {view === "list" ? (
            <div className="space-y-4">
              {results.length === 0 ? (
                <div className="rounded-xl bg-card border border-dashed border-border p-10 text-center">
                  ...
                  <button onClick={() => { setQuery(""); setChips([]); }} className="mt-4 rounded-md bg-primary text-primary-foreground px-4 py-2 text-xs font-semibold">Clear search</button>
                </div>
              ) : results.map((h, i) => (
                <Link key={h.id} to={`/hospital/${h.id}`} onMouseEnter={() => setSelectedId(h.id)} className="block">
...
function FilterPanel({ chips, setChips }) {
  const SPECS = ["Cardiology", "Neurology", "Orthopedics", "Pediatrics", "Oncology"];
  const [distance, setDistance] = useState(5);
  const toggleChip = (label) => chips.includes(label) ? setChips(chips.filter((c) => c !== label)) : setChips([...chips, label]);
  return (
    <div className="rounded-xl bg-card border border-border p-5 mb-4 space-y-5">
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {SPECS.map((s) => { const tag = `Specialty: ${s}`; const on = chips.includes(tag); return (<button key={s} onClick={() => toggleChip(tag)} className={`rounded-full px-3 py-1 text-xs border transition ${on ? "bg-primary text-primary-foreground border-primary" : "bg-card border-border hover:border-primary"}`}>{s}</button>); })}
        </div>
      </div>
      <div>
        ...
        <input type="range" min={1} max={20} value={distance} onChange={(e) => setDistance(Number(e.target.value))} className="w-full accent-primary" />
      </div>
      <div className="flex flex-wrap gap-2">
        {["Has beds", "Rating 4+", "Government", "Private", "Open now"].map((t) => { const on = chips.includes(t); return (<button key={t} onClick={() => toggleChip(t)} className={`rounded-full px-3 py-1 text-xs border transition ${on ? "bg-success text-success-foreground border-success" : "bg-card border-border hover:border-success"}`}>{t}</button>); })}
      </div>
    </div>
  );
}
