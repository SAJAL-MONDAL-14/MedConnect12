...
import { doctors, clinicDoctors, hospitals } from "@/lib/mockData";
...
export default function BookingPage() {
  ...
  useEffect(() => {
    document.title = "Confirm booking — MedConnect";
  }, []);

  const docId = searchParams.get("docId") || "d1";
  const slotTime = searchParams.get("time") || "10:00 AM";
  const dateLabel = searchParams.get("date") || "Today, 2 May";

  // Find doctor in hospital doctors or chamber doctors
  const doc =
    doctors.find((x) => x.id === docId) || clinicDoctors.find((x) => x.id === docId) || doctors[0];
  const isChamber = clinicDoctors.some((x) => x.id === doc.id);
  const hospital = !isChamber ? hospitals.find((h) => h.id === doc.hospitalId) : null;

  const doctorName = doc.name;
  const doctorSpecialty = doc.specialty;
  const doctorInitials = doc.initials;
  const doctorFee = doc.fee;
  const placeName = isChamber ? doc.clinicName : (hospital?.name ?? "North Bengal Medical College");
  const room = isChamber ? doc.address : "OPD Block, Room 4";
  ...
  const submit = () => {
    setSubmitting(true);
    setTimeout(() => navigate("/booking-success"), 900);
  };
        ...
        <div className="flex items-center gap-2 mb-6">
          {[1, 2].map((n, i) => (
            <div key={n} className="flex items-center gap-2 flex-1">
              <div
                className={`h-7 w-7 rounded-full flex items-center justify-center text-xs font-bold ${step >= n ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
              >
                {step > n ? <CheckCircle2 className="h-4 w-4" /> : n}
              </div>
              <span
                className={`text-xs font-medium ${step >= n ? "text-foreground" : "text-muted-foreground"}`}
              >
            ...
            <div className="rounded-xl bg-card border border-border p-5">
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 text-sm">
                <div>
                  ...
                  <div className="font-medium">{dateLabel} 2025</div>
                </div>
                <div>
                  <div className="text-xs text-muted-foreground mb-1">Time</div>
                  <div className="font-medium font-mono">{slotTime}</div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="font-mono text-xs text-muted-foreground">Auto-generated</div>
                </div>
              </div>
            </div>
              ...
              <div className="rounded-xl bg-card border border-border p-5">
                ...
                <div className="space-y-2">
                  <PayOption
                    ...
                    title={isChamber ? "Pay at chamber" : "Pay at clinic"}
                    desc={`Pay ₹${doctorFee} directly at reception`}
                  />
                  <PayOption
                    selected={pay === "online"}
            ...
            <div className="rounded-xl bg-card border border-border p-5">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono font-semibold">₹{doctorFee}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between pt-3 border-t border-border font-semibold">
                  <span>Total</span>
                  <span className="font-mono text-primary font-bold">₹{doctorFee}</span>
                </div>
              </div>
              <div className="mt-4 rounded-md bg-success-soft border border-success/20 px-3 py-2 text-xs text-success">
                {pay === "clinic"
                  ? `Pay ₹${doctorFee} directly at reception`
                  : `₹${doctorFee} will be charged via Razorpay`}
              </div>
...
function PayOption({ selected, onClick, icon, title, desc }) {
  return (
    <button
      onClick={onClick}
      className={`w-full text-left rounded-lg border-2 p-4 flex items-center gap-3 transition ${selected ? "border-primary bg-primary-soft" : "border-border bg-card hover:border-primary/50"}`}
    >
      <div
        className={`h-10 w-10 rounded-md flex items-center justify-center ${selected ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
      >
        {icon}
      </div>
      ...
      <div
        className={`h-5 w-5 rounded-full border-2 flex items-center justify-center ${selected ? "border-primary bg-primary" : "border-border"}`}
      >
