...
const todayAppointments = [
  ...
  },
  {
    ...
    age: 41,
    type: "Follow-up",
    reason: "Post-op checkup after angioplasty",
    paid: true,
    payMode: "At clinic",
    fee: 300,
    status: "Completed",
  ...
  },
  {
    ...
    age: 60,
    type: "New Patient",
    reason: "Chest tightness, left arm pain",
    paid: false,
    payMode: "Pending",
    fee: 500,
    status: "Booked",
...
const reviews = [
  {
    ...
    rating: 5,
    date: "2 days ago",
    comment: "Dr. Sharma explained everything clearly. Best cardiologist in Siliguri.",
  },
  {
    name: "Vikram Roy",
    rating: 5,
    date: "1 week ago",
    comment: "Very professional. Diagnosed my condition quickly and the treatment worked.",
  },
  {
    name: "Sneha Bose",
    rating: 4,
    date: "2 weeks ago",
    comment: "Good consultation. Slight wait time but the care was excellent.",
  },
];
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <aside className="hidden md:flex flex-col w-[220px] shrink-0 text-white bg-gradient-to-b from-success-dark via-success to-primary">
        ...
        <div className="p-4 border-b border-white/10 flex items-center gap-2.5">
          <div className="h-9 w-9 rounded-full bg-white/20 backdrop-blur flex items-center justify-center text-xs font-bold ring-2 ring-white/30">
            RS
          </div>
        ...
        <header className="bg-card border-b border-border h-16 flex items-center justify-between px-6 sticky top-0 z-30">
          <div>
            ...
            <div className="text-xs text-muted-foreground">
              Cardiologist · North Bengal Medical College · OPD Block, Room 4
            </div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="h-8 w-8 rounded-full bg-success text-success-foreground flex items-center justify-center text-xs font-bold">
              RS
            </div>
          </div>
        </header>
        ...
        <div className="relative flex items-start justify-between flex-wrap gap-4">
          <div>
            <div className="text-xs uppercase tracking-wider text-white/70 font-bold">
              Friday, 2 May 2025 · OPD Active
            </div>
            <h2 className="mt-1 text-2xl font-bold">You have {totalToday} patients today</h2>
            {nextPatient && (
              <p className="text-sm text-white/80 mt-1">
                Next: <span className="font-semibold">{nextPatient.patient}</span> at{" "}
                <span className="font-mono">{nextPatient.time}</span> — Token #{nextPatient.token}
              </p>
            )}
            <button
              onClick={() => setView("My Appointments")}
              className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-white text-success text-sm font-semibold hover:bg-white/90 transition"
            >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            delta: `${paidCount}/${totalToday} paid`,
            tone: "success",
            icon: Wallet,
          },
      ...
      <div className="px-6 py-4 border-b border-border flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            ...
            {appts.filter((a) => a.status === "Waiting" || a.status === "In Progress").length}{" "}
            active
          </p>
        </div>
        {compact && setView && (
            ...
            <div className="sticky top-0 bg-card border-b border-border px-6 py-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Token #{selected.token} · {selected.time}
                </p>
              </div>
              <button onClick={() => setSelected(null)} className="p-2 hover:bg-muted rounded-md">
                <X className="h-4 w-4" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-sm text-muted-foreground">
                    Age {selected.age} · {selected.type}
                  </div>
                </div>
              ...
              <div className="rounded-lg bg-secondary p-4 space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-mono font-semibold">₹{selected.fee}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-muted-foreground">Payment</span>
                  <PayChip paid={selected.paid} mode={selected.payMode} />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full rounded-md bg-input border border-border px-3 py-2 text-sm outline-none focus:border-success resize-none"
                />
              </div>
    ...
    <div className="rounded-xl bg-card border border-border p-5">
      ...
      <div className="border-t border-border pt-3 space-y-1.5">
        {[
          ...
          {
            label: "Revenue",
            value: `₹${(weeklyData.reduce((s, d) => s + d.revenue, 0) / 1000).toFixed(1)}k`,
          },
          {
            label: "Payment collected",
            value: `${weeklyData.reduce((s, d) => s + d.paid, 0)} paid`,
          },
        ].map((r) => (
    ...
    <div className="rounded-xl bg-card border border-border p-5">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        <div className="rounded-lg bg-success-soft p-3 border border-success/20">
          ...
          <div className="text-2xl font-bold text-success font-mono mt-1">
        ...
        <div className="rounded-lg bg-warning-soft p-3 border border-warning/20">
          ...
          <div className="text-2xl font-bold text-warning font-mono mt-1">
      ...
      <div className="space-y-2">
        {[
          {
            label: "Online (Razorpay/UPI)",
            count: paid.filter((a) => a.payMode === "Online").length,
            amount: paid.filter((a) => a.payMode === "Online").reduce((s, a) => s + a.fee, 0),
          },
          {
            label: "At clinic (cash/card)",
            count: paid.filter((a) => a.payMode === "At clinic").length,
            amount: paid.filter((a) => a.payMode === "At clinic").reduce((s, a) => s + a.fee, 0),
          },
        ].map((r) => (
          <div
            key={r.label}
            className="flex items-center justify-between text-xs rounded-md bg-secondary px-3 py-2"
          >
...
function AppointmentsView() {
  ...
  const filtered =
    filter === "All" ? todayAppointments : todayAppointments.filter((a) => a.status === filter);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          {
            l: "Completed",
            v: todayAppointments.filter((a) => a.status === "Completed").length,
            tone: "success",
          },
          { l: "Paid", v: todayAppointments.filter((a) => a.paid).length, tone: "warning" },
          {
            l: "No-show",
            v: todayAppointments.filter((a) => a.status === "No-show").length,
            tone: "emergency",
          },
        ].map((s) => (
...
function PatientsView() {
  ...
  const filtered = patientHistory.filter(
    (p) =>
              ...
              {filtered.map((p) => (
                <tr key={p.id} className="hover:bg-secondary/50">
                  ...
                  <td className="px-6 py-3 hidden sm:table-cell">
                    <span className="rounded-full bg-emergency-soft text-emergency px-2 py-0.5 text-[10px] font-bold">
                      {p.bloodGroup}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-3 text-right">
                    <a
                      href={`tel:${p.phone}`}
                      className="p-1.5 rounded hover:bg-success-soft text-success inline-flex"
                    >
    ...
    <div className="space-y-4">
      <div className="rounded-xl bg-card border border-border p-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <button className="rounded-md bg-success text-success-foreground px-3 py-1.5 text-xs font-semibold">
            Save changes
          </button>
        </div>
        <div className="overflow-x-auto">
              ...
              {mySlots.map((row) => (
                <tr key={row.day} className="hover:bg-secondary/50">
                    ...
                    <div className="flex flex-wrap gap-1">
                      {row.slots.length === 0 ? (
                        ...
                        row.slots.map((s) => (
                          <span
                            key={s}
                            className="text-[10px] font-mono bg-primary-soft text-primary rounded px-1.5 py-0.5"
                          >
                            {s}
                          </span>
                        ))
                  ...
                  <td className="px-4 py-3 text-xs text-muted-foreground">
                    {row.slots.length === 0
                      ? "—"
                      : `${row.slots[0]} – ${row.slots[row.slots.length - 1]}`}
                  </td>
      ...
      <div className="rounded-xl bg-card border border-border p-5">
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          ...
          <button className="rounded-md bg-warning text-warning-foreground px-3 py-2 text-sm font-semibold hover:opacity-90">
            Block slot
          </button>
        </div>
      </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {selected.reason} · {new Date().toLocaleDateString()}
                  </p>
                </div>
                  ...
                  {meds.map((m, i) => (
                    ...
                    >
                      <input
                        ...
                        className="rounded-md bg-input border border-border px-3 py-2 text-sm outline-none focus:border-success"
                      />
                      <input
                        value={m.dose}
                        onChange={(e) => updateMed(i, "dose", e.target.value)}
                        placeholder="Dose"
                        className="w-20 rounded-md bg-input border border-border px-3 py-2 text-sm outline-none focus:border-success"
                      />
                      <input
                        value={m.freq}
                      ...
                      <input
                        ...
                        placeholder="Days"
                        className="w-16 rounded-md bg-input border border-border px-3 py-2 text-sm outline-none focus:border-success"
                      />
                      <button
                        onClick={() => removeMed(i)}
                        className="p-2 text-emergency hover:bg-emergency-soft rounded-md"
                      >
                        <X className="h-3.5 w-3.5" />
                      </button>
                    </div>
              ...
              <div>
                ...
                <textarea
                  rows={2}
                  placeholder="e.g. Take after meals, avoid alcohol, follow-up in 2 weeks..."
                  className="w-full rounded-md bg-input border border-border px-3 py-2 text-sm outline-none focus:border-success resize-none"
                />
              </div>
              ...
              <div>
                ...
                <input
                  placeholder="e.g. ECG, Lipid profile, HbA1c"
                  className="w-full rounded-md bg-input border border-border px-3 py-2 text-sm outline-none focus:border-success"
                />
              </div>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <input
                    type="date"
                    className="flex-1 rounded-md bg-input border border-border px-3 py-2 text-sm outline-none focus:border-success"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-0.5">
                {Array.from({ length: 5 }).map((_, j) => (
                  <Star
                    key={j}
                    className={`h-3.5 w-3.5 ${j < r.rating ? "fill-warning text-warning" : "text-muted"}`}
                  />
                ))}
              </div>
            </div>
...
function SettingsView() {
  ...
  return (
    <div className="rounded-xl bg-card border border-border p-6 space-y-4 max-w-2xl">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            ...
            className="w-full rounded-md bg-input border border-border px-3 py-2 text-sm outline-none focus:border-success font-mono"
          />
        </div>
        <div>
          <label className="block text-xs text-muted-foreground mb-1.5 font-medium">
      ...
      <button className="rounded-md bg-success text-success-foreground px-4 py-2 text-sm font-semibold">
        Save changes
      </button>
    </div>
  );
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="rounded-xl bg-card border border-border p-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <span
            className={`rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wider ${sc.bg}`}
          >
            {sc.label}
          </span>
        </div>
          ...
          {timeline.map((s, i) => (
            <div key={i} className="flex items-center flex-1">
              <div className="flex flex-col items-center">
                <div
                  className={`h-7 w-7 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${s.done ? "bg-success text-success-foreground" : s.current ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
                >
                  {s.done ? "✓" : i + 1}
                </div>
                <div
                  className={`text-[9px] mt-1 text-center leading-tight max-w-[56px] ${s.current ? "text-success font-semibold" : s.done ? "text-muted-foreground" : "text-muted-foreground"}`}
                >
                  {s.label}
                </div>
              </div>
              {i < timeline.length - 1 && (
                <div className={`flex-1 h-0.5 mx-1 mb-4 ${s.done ? "bg-success" : "bg-border"}`} />
              )}
            </div>
          ))}
        </div>
        {onboardingStatus === "approved" && (
          <div className="mt-4 rounded-lg bg-success-soft border border-success/20 px-4 py-3 text-sm text-success flex items-center gap-2">
            <span className="text-lg">🎉</span> Your profile is verified and live. Patients can now
            find and book you.
          </div>
        )}
          ...
          <div className="bg-card border border-border rounded-2xl p-5 max-w-sm shadow-card">
            ...
            <div className="mt-4 flex items-center justify-between text-sm">
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-muted-foreground">(184)</span>
              </div>
              <div className="text-right">
                <div className="font-bold">₹{fee}</div>
                <div className="text-[10px] text-muted-foreground">Follow-up ₹{followupFee}</div>
              </div>
            </div>
            <div className="mt-3 flex items-center gap-1.5 text-xs text-muted-foreground">
              <span className="text-primary">🏥</span>
              <span>
                at <span className="text-foreground font-medium">North Bengal Medical College</span>
              </span>
            </div>
      ...
      <div className="rounded-xl bg-card border border-border p-6">
        ...
        <div className="space-y-3">
          {[
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={`h-8 w-8 rounded-full flex items-center justify-center ${d.status === "verified" ? "bg-success-soft text-success" : "bg-warning-soft text-warning"}`}
                >
                  {d.status === "verified" ? "✓" : "!"}
                </div>
                <div>
              ...
              <span
                className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${d.status === "verified" ? "bg-success-soft text-success" : "bg-warning-soft text-warning"}`}
              >
                {d.status}
              </span>
            </div>
        ...
        <label className="mt-4 flex items-center gap-2 text-xs text-primary font-medium cursor-pointer hover:underline">
          <input type="file" className="hidden" />+ Upload additional document
        </label>
      </div>
    </div>
  );
