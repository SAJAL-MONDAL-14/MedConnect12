    ...
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <div className="hidden lg:flex flex-col justify-between p-10 text-white relative overflow-hidden bg-gradient-to-br from-success-dark via-success to-primary">
        ...
        <div className="relative">
          ...
          <p className="mt-4 text-white/70 max-w-md">
            See every booking, patient note, payment status, and slot — all in one place built for
            hospital doctors.
          </p>
          ...
          <div className="mt-8 space-y-2">
            ...
            {[
              ...
              <div key={s} className="flex items-center gap-2">
                <div
                  className={`h-5 w-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${i === 5 ? "bg-success text-success-foreground" : i < 3 ? "bg-white/20 text-white" : "bg-white/10 text-white/50"}`}
                >
                  {i < 3 ? "✓" : i + 1}
                </div>
                <span
                  className={`text-xs ${i === 5 ? "text-success font-semibold" : i < 3 ? "text-white" : "text-white/50"}`}
                >
                  {s}
                </span>
              </div>
      ...
      <div className="flex items-center justify-center p-8 bg-background">
        <div className="w-full max-w-sm">
          ...
          <div className="text-[10px] uppercase tracking-[0.2em] text-success font-bold mb-2">
          ...
          {tab === "login" && (
            <>
              ...
              <p className="text-sm text-muted-foreground mt-1 mb-5">
                Access your OPD schedule, bookings & patient records.
              </p>
              ...
              <div className="mt-3 flex items-center gap-2 p-3 rounded-lg bg-warning-soft border border-warning/20">
                <button
                  onClick={() => setIsFirstLogin((f) => !f)}
                  className={`relative inline-flex h-5 w-9 rounded-full transition shrink-0 ${isFirstLogin ? "bg-warning" : "bg-muted"}`}
                >
                  <span
                    className={`absolute top-0.5 h-4 w-4 rounded-full bg-card shadow transition-all ${isFirstLogin ? "left-[18px]" : "left-0.5"}`}
                  />
                </button>
                <span className="text-xs text-warning font-medium">
                  {isFirstLogin
                    ? "First-time login (activation link) → will show profile completion"
                    : "Returning doctor → goes to dashboard"}
                </span>
              </div>
          ...
          {tab === "activate" && (
            <>
              ...
              <p className="text-sm text-muted-foreground mt-1 mb-5">
                Use the activation link sent to your email, or enter your token below.
              </p>
              <div className="rounded-lg bg-primary-soft border border-primary/20 px-4 py-3 text-xs text-primary mb-4 flex items-start gap-2">
                ...
                <span className="font-semibold mx-1">MedConnect</span> with your activation link.
                The link will automatically log you in and redirect you to complete your profile.
              </div>
              <div className="space-y-3">
                <div>
                  <label className="block text-xs text-muted-foreground mb-1.5 font-medium">
              ...
              <button
                onClick={() => navigate("/hospital-doctor/complete-profile")}
          ...
          <div className="mt-6 p-3 rounded-lg bg-secondary border border-border text-xs">
            ...
            <div className="flex flex-wrap gap-x-3 gap-y-1 text-muted-foreground">
              <Link to="/staff/login" className="hover:text-foreground underline">
                Hospital staff
              </Link>
              <Link to="/private-chamber/login" className="hover:text-foreground underline">
                Chamber doctor
              </Link>
