    ...
    <div className="flex items-center mb-8 overflow-x-auto pb-1 gap-0">
      {STEPS.map((s, i) => (
        <div key={s.n} className="flex items-center shrink-0">
          ...
          {i < STEPS.length - 1 && (
            <div
              className={`h-0.5 w-8 sm:w-14 mx-1 mb-4 rounded transition-all ${current > s.n ? "bg-success" : "bg-border"}`}
            />
          )}
        </div>
      ))}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        ...
        {readOnly && (
          <span className="text-[10px] rounded-full bg-primary-soft text-primary px-2 py-0.5 font-semibold">
            Pre-filled by admin
          </span>
        )}
      ...
      <div
        className={`flex items-center gap-2 rounded-md border px-3 transition ${
          readOnly
...
function UploadBox({ label, desc, value, onChange, required }) {
  return (
    <div>
      ...
      <label
        className={`flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed p-5 cursor-pointer transition ${
          value
      ...
      >
        ...
        {value ? (
          <>
            ...
            <span className="text-xs font-semibold text-success text-center truncate max-w-full px-2">
              {value}
            </span>
...
function ReviewRow({ label, value, highlight }) {
  return (
    <div
      className={`flex items-start justify-between px-4 py-3 text-sm border-b border-border last:border-0 ${highlight ? "bg-primary-soft/30" : ""}`}
    >
...
export default function HospitalDoctorCompleteProfile() {
  ...
  useEffect(() => {
    document.title = "Complete your profile — MedConnect";
  }, []);
        ...
        <div className="max-w-lg w-full bg-card border border-border rounded-2xl p-8 shadow-elevated">
          <div className="text-center mb-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              ...
              <span className="font-semibold text-foreground">{ADMIN_PRE_FILLED.hospital}</span>.
              You'll be notified by email at every stage.
            </p>
          </div>
            ...
            {[
              ...
              <div key={i} className="flex items-start gap-3">
                <div className="flex flex-col items-center">
                  ...
                  {i < arr.length - 1 && (
                    <div className={`w-0.5 h-6 ${s.done ? "bg-success" : "bg-border"}`} />
                  )}
                </div>
                <div className="pb-2">
                  <div
                    className={`text-sm font-medium ${s.current ? "text-primary" : s.done ? "text-foreground" : "text-muted-foreground"}`}
                  >
                    {s.label}
                  </div>
          ...
          <div className="rounded-lg bg-primary-soft border border-primary/20 px-4 py-3 text-xs text-primary mb-5">
            While under review, you can still log in to your dashboard to view your profile and
            track approval status.
          </div>

          <div className="grid grid-cols-2 gap-3">
            <Link
              to="/hospital-doctor/login"
      ...
      <div className="bg-gradient-to-br from-primary-soft via-background to-background border-b border-border">
        <div className="max-w-3xl mx-auto px-6 py-8">
          <div className="inline-flex items-center gap-2 text-xs px-3 py-1 rounded-full bg-card border border-border text-muted-foreground mb-3">
            <ShieldCheck className="h-3.5 w-3.5 text-success" /> First-time login — account
            activation
          </div>
          ...
          <p className="text-sm text-muted-foreground mt-1.5 max-w-2xl">
            Welcome, {ADMIN_PRE_FILLED.fullName.split(" ").slice(0, 2).join(" ")}! Fields marked{" "}
            <span className="rounded-full bg-primary-soft text-primary text-[10px] font-semibold px-2 py-0.5">
              Pre-filled by admin
            </span>{" "}
            were entered by your hospital admin — you can update them if needed.
          </p>
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <div className="bg-card border border-border rounded-2xl p-6 md:p-8">
            ...
            <div className="space-y-5">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Basic information about you. Some fields are pre-filled by your hospital admin.
                </p>
              </div>
              ...
              <Field
                ...
                value={address}
                onChange={setAddress}
                placeholder="123, Hill Cart Road, Siliguri, WB 734001"
              />
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Your medical credentials and practice information. Update if any details differ.
                </p>
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-muted-foreground mb-1.5 font-medium">
                    Specialty<span className="text-emergency ml-0.5">*</span>
                  </label>
                  <select
                    value={specialty}
              ...
              <div>
                <label className="block text-xs text-muted-foreground mb-1.5 font-medium">
                  ...
                  <span className="ml-2 text-[10px] rounded-full bg-primary-soft text-primary px-2 py-0.5 font-semibold">
                    Pre-filled by admin
                  </span>
                </label>
                <input
                ...
                <p className="text-[11px] text-muted-foreground mt-1">
                  Comma-separated. Add all relevant degrees and fellowships.
                </p>
              </div>
              <Field
              ...
              <div className="flex items-center gap-3 p-4 rounded-xl bg-secondary border border-border">
                <button
                  onClick={() => setIsSurgeon((s) => !s)}
                  className={`relative inline-flex h-6 w-11 rounded-full transition shrink-0 ${isSurgeon ? "bg-success" : "bg-muted"}`}
                >
                  <span
                    className={`absolute top-0.5 h-5 w-5 rounded-full bg-card shadow transition-all ${isSurgeon ? "left-[22px]" : "left-0.5"}`}
                  />
                </button>
                <div>
                  <div className="text-sm font-semibold">I am a surgeon</div>
                  <div className="text-xs text-muted-foreground">
                    Surgeons receive a "Surgeon" badge on their profile and can be assigned to OT
                    slots by the admin.
                  </div>
                </div>
              </div>
          ...
          {step === 3 && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Used for consultation fee payouts. All data is encrypted and never shared with
                  patients.
                </p>
              </div>
              <div className="rounded-lg bg-primary-soft border border-primary/20 px-4 py-3 text-xs text-primary flex items-start gap-2">
                ...
                Your bank details are securely stored and only used by MedConnect finance team for
                payouts. We never share this with hospitals or patients.
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                <Field
                  label="Bank name"
              ...
              <div className="border-t border-border pt-5">
                ...
                <p className="text-xs text-muted-foreground mb-4">
                  Provide at least one identity document. Both PAN and passport are accepted.
                </p>
                <div className="grid sm:grid-cols-2 gap-4">
                  <Field
                    label="PAN number"
                ...
                {!pan && !passport && (
                  <p className="text-xs text-warning mt-2 flex items-center gap-1.5">
                    <AlertCircle className="h-3.5 w-3.5" /> Please provide either PAN or passport
                    number to continue.
                  </p>
                )}
              </div>
          ...
          {step === 4 && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Required documents for identity and credential verification. Items marked{" "}
                  <span className="text-emergency font-semibold">*</span> are mandatory.
                </p>
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                <UploadBox
                  label="Profile photograph"
                  desc="Clear headshot on white background, professional attire"
                  value={photo}
                  onChange={setPhoto}
                  required
                />
                <UploadBox
                  label="MCI / Council registration certificate"
                  desc="State or national medical council registration"
                  value={regDoc}
                  onChange={setRegDoc}
                  required
                />
                <UploadBox
                  ...
                  desc="MBBS, MD, MS, DM or equivalent"
                  value={degreeDoc}
                ...
                <UploadBox
                  ...
                  desc="Aadhar card, Passport or Voter ID"
                  value={idDoc}
                  onChange={setIdDoc}
                />
                <UploadBox
                  label="Bank passbook / cancelled cheque"
                  desc="For payout verification"
                  value={bankDoc}
                  onChange={setBankDoc}
                />
                <UploadBox
                  label="Digital signature"
                  desc="Sign on white paper, scan/photograph (PNG/JPG)"
                  value={sigDoc}
                  onChange={setSigDoc}
                />
              </div>
              {(!photo || !regDoc || !degreeDoc) && (
                <div className="rounded-lg bg-warning-soft border border-warning/20 px-4 py-3 text-xs text-warning flex items-center gap-2">
                  ...
                  Please upload your profile photograph, registration certificate, and degree
          ...
          {step === 5 && (
            <div className="space-y-5">
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Review all your details carefully before submitting for admin review.
                </p>
              </div>
              ...
              <div className="rounded-xl border border-border overflow-hidden">
                ...
                <ReviewRow
                  label="Account no."
                  value={accountNo ? `••••${accountNo.slice(-4)}` : ""}
                />
                ...
                {[
                  ...
                  >
                    <span className="text-muted-foreground">
                      {d.l}
                      {d.req && <span className="text-emergency ml-0.5">*</span>}
                    </span>
                    {d.v ? (
                      ...
                      <span
                        className={`text-xs font-medium ${d.req ? "text-emergency" : "text-muted-foreground"}`}
                      >
                        {d.req ? "Required" : "Not uploaded"}
                      </span>
                    )}
                  </div>
                ))}
              ...
              <div className="rounded-lg bg-warning-soft border border-warning/20 px-4 py-3 text-xs text-warning flex items-start gap-2">
                ...
                By submitting, you confirm that all information provided is accurate. False or
                misleading information may result in rejection or account termination.
              </div>
            </div>
          )}
          ...
          <div className="mt-8 flex items-center justify-between">
            <button
              onClick={() =>
                step > 1 ? setStep((s) => s - 1) : navigate("/hospital-doctor/login")
              }
