    ...
    <div className="flex items-center mb-8 overflow-x-auto pb-1">
      {STEPS.map((s, i) => (
        <div key={s.n} className="flex items-center shrink-0">
          ...
          {i < STEPS.length - 1 && (
            <div
              className={`h-0.5 w-8 sm:w-12 mx-1 mb-4 rounded transition-all ${current > s.n ? "bg-success" : "bg-border"}`}
            />
          )}
        </div>
      ))}
...
function Field({ label, value, onChange, placeholder, type = "text", required, icon, readOnly }) {
  return (
    <div>
      ...
      <div
        className={`flex items-center gap-2 rounded-md border px-3 transition ${readOnly ? "border-border bg-secondary" : "border-border bg-input focus-within:border-primary"}`}
      >
      ...
      >
        ...
        {value ? (
          <>
            ...
            <div className="text-xs font-medium text-success text-center truncate max-w-full px-2">
              {value}
            </div>
          </>
        ) : (
      ...
      <div className="min-h-screen bg-background flex items-center justify-center px-6 py-12">
        <div className="max-w-md w-full bg-card border border-border rounded-2xl p-8 text-center shadow-elevated">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            ...
            <span className="font-semibold text-foreground">{PRE_FILLED.hospital}</span> for review.
            You'll be notified by email at each stage.
          </p>
            ...
            {[
              ...
              <div key={i} className="flex items-start gap-3">
                <div className="flex flex-col items-center">
                  ...
                  {i < arr.length - 1 && (
                    <div className={`w-0.5 h-5 ${s.done ? "bg-success" : "bg-border"}`} />
                  )}
                </div>
                <div
                  className={`text-sm py-0.5 leading-relaxed ${
                    s.current
          ...
          <div className="mt-6 p-3 rounded-lg bg-primary-soft border border-primary/20 text-xs text-primary text-left">
            You can log in anytime to check your approval status and update your profile if changes
            are requested.
          </div>
      ...
      <div className="bg-gradient-to-br from-primary-soft via-background to-background border-b border-border">
        <div className="max-w-3xl mx-auto px-6 py-8">
          ...
          <p className="text-sm text-muted-foreground mt-1 max-w-xl">
            Your profile will be reviewed by the hospital admin before your account is fully
            activated. Fields marked with{" "}
            <span className="text-warning font-semibold">Pre-filled</span> were entered by the admin
            — you can update them.
          </p>
        </div>
      </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-6 md:p-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <Field
                  ...
                  value={address}
                  onChange={setAddress}
                  placeholder="123, Hill Cart Road, Siliguri"
                />
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 p-4 rounded-xl bg-secondary border border-border">
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    You'll be tagged with a "Surgeon" badge and can be assigned to OT slots by the
                    admin.
                  </div>
                </div>
              </div>
              <div>
                <label className="block text-xs text-muted-foreground mb-1.5 font-medium">
                  About your practice (shown on public profile)
                </label>
                <textarea
                  value={bio}
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              <div className="rounded-lg bg-primary-soft border border-primary/20 px-4 py-3 text-xs text-primary">
                Your bank details are used for consultation fee payouts. This information is
                encrypted and never shared with patients.
              </div>
              <div className="grid sm:grid-cols-2 gap-4">
                <Field
                  label="Bank name"
              ...
              <div className="border-t border-border pt-4">
                ...
                <p className="text-xs text-muted-foreground mb-3">
                  At least one of PAN or passport is required.
                </p>
                <div className="grid sm:grid-cols-2 gap-4">
                  <Field
                    label="PAN number"
          ...
          {step === 4 && (
            <div className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                Items marked * are mandatory for approval.
              </p>
              <div className="grid sm:grid-cols-2 gap-4">
                <UploadBox
                  label="Profile photograph"
                  desc="Clear headshot, professional attire"
                  value={photo}
                  onChange={setPhoto}
                  required
                />
                <UploadBox
                  label="MCI / Council registration"
                  desc="Medical council certificate"
                  value={regDoc}
                  onChange={setRegDoc}
                  required
                />
                <UploadBox
                  ...
                  desc="MBBS / MD / MS or highest degree"
                  value={degreeDoc}
                ...
                <UploadBox
                  ...
                  desc="Signature on white paper (PNG/JPG)"
                  value={sigDoc}
                  onChange={setSigDoc}
                />
              </div>
              {(!photo || !regDoc || !degreeDoc) && (
                <p className="text-xs text-warning flex items-center gap-1.5">
                  <AlertCircle className="h-3.5 w-3.5" /> Please upload all required (*) documents
                  to continue.
                </p>
              )}
            </div>
          ...
          {step === 5 && (
            <div className="space-y-4">
              ...
              <div className="rounded-lg bg-warning-soft border border-warning/20 px-4 py-3 text-xs text-warning flex items-start gap-2">
                ...
                By submitting, you confirm all details are accurate. False information may result in
                account termination.
              </div>
            </div>
          )}
