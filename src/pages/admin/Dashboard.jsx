      ...
      <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 rounded-md border border-border bg-input px-3 w-64">
              ...
              <input
                ...
                className="bg-transparent py-1.5 text-sm outline-none flex-1"
              />
            </div>
            <button className="relative p-2 rounded-md hover:bg-muted">
              <Bell className="h-4 w-4" />
        ...
        <main className="p-6 space-y-6">
          ...
          {view === "Labs" && (
            <PlaceholderView
              title="Labs"
              desc="Manage lab tenants and home-collection availability."
            />
          )}
          {view === "Doctors" && (
            <PlaceholderView title="Doctors" desc="All registered doctors across the platform." />
          )}
          {view === "Clinic Applications" && <ClinicApplicationsView />}
          {view === "Patients" && (
            <PlaceholderView title="Patients" desc="Search and manage patient accounts." />
          )}
          {view === "Staff" && (
            <PlaceholderView
              title="Staff accounts"
              desc="Hospital staff users with portal access."
            />
          )}
          {view === "SOS Events" && <SOSEventsView />}
          {view === "Bookings" && (
            <PlaceholderView title="All bookings" desc="Cross-tenant bookings ledger." />
          )}
          {view === "Billing" && <BillingView />}
          {view === "Settings" && (
            <PlaceholderView
              title="Platform settings"
              desc="Global config, integrations, branding."
            />
          )}
        </main>
      </div>
    ...
    <>
        ...
        <div className="relative flex items-start justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-sm text-white/80 mt-1">
              48 hospitals, 1,284 bookings today, 7 SOS events resolved.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <div className="px-4 py-2 rounded-lg bg-white/15 backdrop-blur border border-white/20">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((s) => (
          <div
            key={s.label}
            className="rounded-xl bg-card border border-border p-5 shadow-card hover:shadow-lg hover:-translate-y-0.5 transition"
          >
            <div className="flex items-center justify-between">
              <div
                className={`h-10 w-10 rounded-lg flex items-center justify-center bg-${s.tone}-soft text-${s.tone}`}
              >
        ...
        {events.map((e) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm font-medium truncate">{e.name}</div>
              <div className="text-xs text-muted-foreground inline-flex items-center gap-1">
                <span className="font-mono">{e.time}</span> <ChevronRight className="h-3 w-3" />{" "}
                {e.hospital} · {e.eta}
              </div>
            </div>
            <span
...
function HospitalsView() {
  ...
  const onboard = () => {
    ...
    setTenants([
      ...tenants,
      { id: Date.now(), name, type, beds: null, icu: null, bookings: null, status: "Pending" },
    ]);
    setName("");
    setType("Private");
  ...
  const approve = (id) =>
    setTenants(
      tenants.map((t) =>
        t.id === id ? { ...t, status: "Active", beds: 0, icu: 0, bookings: 0 } : t,
      ),
    );
    ...
    <div className="rounded-xl bg-card border border-border overflow-hidden">
      <div className="px-6 py-4 border-b border-border flex items-center justify-between">
        ...
        <button
          ...
          className="rounded-md bg-primary text-primary-foreground px-3 py-1.5 text-xs font-semibold hover:bg-primary-dark inline-flex items-center gap-1"
        >
          <Plus className="h-3.5 w-3.5" /> Onboard hospital
        </button>
      </div>
      {showOnboard && (
        <div className="px-6 py-4 bg-secondary border-b border-border grid sm:grid-cols-3 gap-3">
          <input
            ...
            className="rounded-md bg-card border border-border px-3 py-2 text-sm outline-none focus:border-primary"
          />
          <select
            value={type}
          ...
          <div className="flex gap-2">
            <button
              onClick={onboard}
              className="flex-1 rounded-md bg-success text-success-foreground px-3 py-2 text-xs font-semibold"
            >
              Submit
            </button>
            <button
                ...
                <td className="px-6 py-3 text-right">
                  {t.status === "Pending" ? (
                    <button
                      ...
                      className="text-success font-semibold text-xs hover:underline"
                    >
                      Approve
                    </button>
                  ) : (
...
function PendingApprovals() {
  ...
  return (
    <div className="rounded-xl bg-card border border-border p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-semibold">Pending approvals</h2>
        <span className="rounded-full bg-warning-soft text-warning px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider">
          ...
          {pending.map((p) => (
            <div key={p.id} className="rounded-lg bg-warning-soft border border-warning/20 p-4">
              ...
              <div className="mt-3 flex gap-2">
                <button
                  onClick={() => remove(p.id)}
                  className="flex-1 rounded-md bg-success text-success-foreground py-1.5 text-xs font-semibold hover:opacity-90"
                >
                  Approve
                </button>
                <button
                  onClick={() => remove(p.id)}
                  className="flex-1 rounded-md border border-emergency text-emergency py-1.5 text-xs font-semibold hover:bg-emergency-soft"
                >
                  Reject
                </button>
              </div>
        ...
        <tbody className="divide-y divide-border">
          {rows.map((r) => (
            <tr key={r.month} className="hover:bg-secondary/50">
              ...
              <td className="px-6 py-3 font-mono">{r.commission}</td>
              <td className="px-6 py-3">
                <span className="rounded-full bg-success-soft text-success px-2 py-0.5 text-[10px] font-bold uppercase">
                  {r.status}
                </span>
              </td>
            </tr>
        ...
        <div className="px-6 py-4 border-b border-border flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">
              Review and approve independent doctor clinics joining the platform.
            </p>
          </div>
          <div className="flex gap-1 rounded-md bg-secondary p-1">
            {["all", "pending", "approved", "rejected"].map((f) => (
        ...
        {filtered.length === 0 ? (
          <div className="px-6 py-16 text-center">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Submitted clinics will appear here for review.
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
