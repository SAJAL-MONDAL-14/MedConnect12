  ...
  return (
    <div className="rounded-xl bg-card border border-border p-5 hover:shadow-lift transition">
      <div className="flex items-start justify-between gap-4 mb-4">
        <div className="flex items-center gap-3">
          <div
            className={`h-12 w-12 rounded-full bg-${d.avatarColor} text-${d.avatarColor}-foreground flex items-center justify-center font-bold`}
          >
            {d.initials}
          </div>
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              {d.specialty} · {d.experience} years
            </p>
            <div className="mt-1">
          ...
          {timeSlots.map((s) => {
            if (s.state === "taken")
              return (
                <button
                  key={s.time}
                  disabled
                  className="rounded-md bg-muted text-text-muted py-1.5 text-xs line-through cursor-not-allowed"
                >
                  {s.time}
                </button>
              );
            ...
            return (
              <button
                ...
                className={`rounded-md py-1.5 text-xs font-medium transition inline-flex items-center justify-center gap-1 ${isSel ? "bg-primary text-primary-foreground" : "bg-card border border-primary/30 text-primary hover:bg-primary-soft"}`}
              >
                {isSel && <CheckCircle2 className="h-3 w-3" />} {s.time}
              </button>
            );
      ...
      <Link
        to="/booking"
        className="mt-3 block text-center rounded-md bg-primary text-primary-foreground py-2.5 text-sm font-semibold hover:bg-primary-dark transition"
      >
        Book {selected}
      </Link>
    </div>
...
function TabBtn({ active, onClick, icon: Icon, label, count }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium border-b-2 whitespace-nowrap transition ${active ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}
    >
      <Icon className="h-4 w-4" /> {label}
      {count != null && (
        <span
          className={`text-xs rounded-full px-1.5 py-0.5 ${active ? "bg-primary-soft text-primary" : "bg-muted text-text-muted"}`}
        >
          {count}
        </span>
      )}
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4">
      ...
      <div className="text-[11px] text-text-muted mt-1.5">
        {total ? `${value} available of ${total}` : "Status"}
      </div>
    </div>
  );
}
...
function HospitalTabs({ hospital, hospitalDoctors }) {
  ...
  return (
    <div>
      <div className="flex gap-1 overflow-x-auto border-b border-border mb-5 -mx-1 px-1">
        <TabBtn
          active={tab === "overview"}
        ...
        />
        <TabBtn
          active={tab === "specialties"}
          onClick={() => setTab("specialties")}
          icon={Heart}
          label="Specialties"
          count={hospital.specialties.length}
        />
      </div>
        ...
        <div className="space-y-5">
          <div className="rounded-xl bg-card border border-border p-5">
            <h3 className="font-semibold mb-3 flex items-center gap-2">
              <Building2 className="h-4 w-4 text-primary" /> About this hospital
            </h3>
            <p className="text-sm text-muted-foreground leading-relaxed">
              {hospital.name} is a {hospital.type.toLowerCase()}{" "}
              {hospital.beds.icuTotal > 8 ? "multi-specialty" : "general"} hospital located in{" "}
              {hospital.city}. The facility offers round-the-clock emergency care,{" "}
              {hospital.specialties.length}+ specialties, and{" "}
              {hospital.beds.generalTotal + hospital.beds.icuTotal} total inpatient beds with live
              availability updates.
            </p>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <StatBox
              value={hospital.beds.general}
              total={hospital.beds.generalTotal}
              label="General Beds"
              tone="primary"
              icon={BedDouble}
            />
            <StatBox
              value={hospital.beds.icu}
              total={hospital.beds.icuTotal}
              label="ICU Beds"
              tone="emergency"
              icon={Heart}
            />
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            {services.slice(0, 6).map((s) => (
              ...
              >
                <div
                  className={`h-9 w-9 rounded-md flex items-center justify-center ${s.on ? "bg-success-soft text-success" : "bg-muted text-text-muted"}`}
                >
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            ...
            <p className="text-xs text-muted-foreground mb-4">
              Bed availability updates in real time. For emergencies, reserve instantly — no payment
              needed upfront.
            </p>
            <div className="grid sm:grid-cols-2 gap-3">
              <Link
                to="/booking"
                className="rounded-md bg-primary text-primary-foreground py-2.5 text-sm font-semibold text-center hover:bg-primary-dark"
              >
                Reserve general bed
              </Link>
              <a
                href={`tel:${hospital.emergencyPhone || "+91 353 256 5100"}`}
                className="rounded-md border border-emergency text-emergency py-2.5 text-sm font-semibold text-center hover:bg-emergency-soft"
              >
                Reserve ICU bed (Call SOS)
              </a>
            </div>
          </div>
          <div className="rounded-xl border border-border bg-card p-5">
            <h3 className="font-semibold mb-3 flex items-center gap-2">
              <Clock className="h-4 w-4 text-primary" /> Updated {hospital.updatedMinAgo} min ago
            </h3>
            <ul className="text-sm space-y-2 text-muted-foreground">
              <li className="flex justify-between">
                ...
                <span
                  className={
                    hospital.beds.ambulance ? "text-success font-medium" : "text-text-muted"
                  }
                >
                  {hospital.beds.ambulance ? "Available 24/7" : "Unavailable"}
                </span>
              </li>
              <li className="flex justify-between">
                <span>Operation theatre</span>
                <span className={hospital.beds.ot ? "text-success font-medium" : "text-text-muted"}>
                  {hospital.beds.ot ? "Active" : "Closed"}
                </span>
              </li>
              <li className="flex justify-between">
                <span>Blood bank</span>
                <span
                  className={hospital.bloodBank ? "text-success font-medium" : "text-text-muted"}
                >
                  {hospital.bloodBank ? "On-site" : "Not on-site"}
                </span>
              </li>
            </ul>
          </div>
      ...
      {tab === "services" && (
        <div className="grid sm:grid-cols-2 gap-3">
          {services.map((s) => {
            const routeMap = {
              "24/7 Emergency": `tel:${hospital.emergencyPhone || "+91 353 256 5100"}`,
              Ambulance: `tel:${hospital.phone || "+91 353 256 5000"}`,
              "Operation Theatre": "/booking",
              "Blood Bank": "/booking",
              Pharmacy: "/booking",
              "ICU/CCU": "/booking",
              "Diagnostics & Lab": "/booking",
            ...
            const isExternal = to.startsWith("tel:") || to.startsWith("mailto:");
            const Wrap = s.on ? (isExternal ? "a" : Link) : "div";
            const wrapProps = s.on ? (isExternal ? { href: to } : { to }) : {};
            return (
              <Wrap
                key={s.label}
                {...wrapProps}
                className={`flex items-center gap-3 rounded-lg border border-border bg-card p-4 transition ${s.on ? "hover:border-primary hover:shadow-card cursor-pointer" : "opacity-70"}`}
              >
                <div
                  className={`h-10 w-10 rounded-lg flex items-center justify-center ${s.on ? "bg-success-soft text-success" : "bg-muted text-text-muted"}`}
                >
                ...
                <div className="flex-1">
                  <div className="text-sm font-medium">{s.label}</div>
                  <div className="text-[11px] text-muted-foreground">
                    {s.on ? "Available at this facility" : "Not available"}
                  </div>
                </div>
                {s.on ? (
          ...
          <div className="rounded-xl border border-border bg-card p-4 flex items-center justify-between">
            <div>
              <h3 className="font-semibold flex items-center gap-2">
                <FlaskConical className="h-4 w-4 text-primary" /> In-house Diagnostics
              </h3>
              <p className="text-xs text-muted-foreground mt-0.5">
                NABL-accredited lab · Reports within 4–24 hrs
              </p>
            </div>
            <Link to="/booking" className="text-sm font-medium text-primary hover:underline">
              Book test →
            </Link>
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-secondary text-xs text-muted-foreground">
                <tr>
                  ...
                  <th className="text-left p-3 font-medium hidden sm:table-cell">Sample</th>
              ...
              <tbody className="divide-y divide-border">
                {labTests.slice(0, 6).map((t) => (
                  <tr
                    ...
                    onClick={() => navigate(`/booking?test=${encodeURIComponent(t.name)}`)}
                  >
                    ...
                    <td className="p-3 text-right">
                      <Link
                        to={`/booking?test=${encodeURIComponent(t.name)}`}
                        onClick={(e) => e.stopPropagation()}
                        className="inline-flex items-center gap-1 rounded-md bg-primary text-primary-foreground px-3 py-1.5 text-xs font-semibold hover:bg-primary-dark"
                      >
                        Book
                      </Link>
                    </td>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {hospital.specialties.map((sp) => {
            ...
            return (
              <div
                key={sp}
                className="rounded-lg border border-border bg-card p-4 flex items-center gap-3 hover:shadow-card transition"
              >
        ...
        <div className="max-w-[1200px] mx-auto px-6 pt-6 pb-10">
          ...
          <div className="mt-7 grid grid-cols-3 gap-3 max-w-2xl">
            {[
              ...
              <div
                key={b.label}
                className="rounded-lg bg-white/10 backdrop-blur border border-white/20 p-4"
              >
                <div className="flex items-center gap-2">
                  <span className="text-3xl font-bold text-white">{b.value}</span>
                  <span className="live-dot" />
            ...
            <div className="divide-y divide-border">
              {[
                ...
                { icon: Siren, label: "Emergency", value: hospital.emergencyPhone },
                {
                  icon: Ambulance,
                  label: "Ambulance",
                  value: hospital.beds.ambulance ? "Available 24/7" : "Not available",
                },
                {
                  icon: Droplet,
                  label: "Blood bank",
                  value: hospital.bloodBank ? "On-site" : "Not on-site",
                },
              ].map((row) => (
                <div key={row.label} className="flex items-center justify-between py-3 text-sm">
                  <span className="inline-flex items-center gap-2 text-muted-foreground">
              ...
              {reviews.map((r) => (
                <div key={r.id} className="rounded-xl bg-card border border-border p-5">
                  ...
                  <div className="text-warning text-xs mb-2">
                    {"★".repeat(r.rating)}
                    <span className="text-warning/30">{"★".repeat(5 - r.rating)}</span>
                  </div>
                  <p className="text-sm text-muted-foreground leading-relaxed">{r.comment}</p>
                </div>
              ))}
          ...
          <div className="rounded-xl bg-card border border-border p-5 shadow-card">
            ...
            <div className="rounded-md bg-secondary p-3 text-sm space-y-1">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{hospitalDoctors[0]?.name ?? "Select"}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Slot</span>
                <span className="font-medium font-mono">Today 10:00 AM</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Fee</span>
                <span className="font-mono font-semibold">₹{hospitalDoctors[0]?.fee ?? 500}</span>
              </div>
            </div>
            <Link
              to="/booking"
              className="mt-4 block text-center rounded-md bg-primary text-primary-foreground py-2.5 text-sm font-semibold hover:bg-primary-dark transition"
            >
              Continue to booking
            </Link>
            <a
              href={`tel:${hospital.emergencyPhone || "+91 353 256 5100"}`}
              className="mt-2 block text-center rounded-md border border-emergency text-emergency py-2.5 text-sm font-semibold hover:bg-emergency-soft transition"
            >
