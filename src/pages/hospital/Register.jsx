...
export default function HospitalRegister() {
  ...
  useEffect(() => {
    document.title = "Register your hospital — MedConnect";
  }, []);
  ...
  const submit = () => {
    ...
    if (finalApp.profile.adminEmail) {
      sendMockEmail({
        to: finalApp.profile.adminEmail,
        subject: t.subject,
        body: t.body,
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        <div className="rounded-xl border border-border bg-card p-6 sm:p-8">
          ...
          {STEPS[step].id === "photos" && (
            <PhotosStep app={app} update={update} onZoom={setZoomSrc} />
          )}
...
function canSubmit(app) {
  return ["profile", "facility", "documents", "photos", "location", "verify"].every((s) =>
    validateStep(s, app),
  );
}
  ...
  return (
    <>
      <StepHeader
        ...
        subtitle="Official identity and contact details."
      />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Hospital name" required>
          <input className={inputCls} value={p.hospitalName || ""} onChange={set("hospitalName")} />
        </Field>
        ...
        <Field label="Hospital Registration Number" required>
          <input className={inputCls} value={p.regNumber || ""} onChange={set("regNumber")} />
        </Field>
        <Field label="Clinical Establishment Reg. No." required>
          <input
            className={inputCls}
            value={p.clinicalRegNumber || ""}
            onChange={set("clinicalRegNumber")}
          />
        </Field>
        <Field label="NABH Accreditation No.">
          <input className={inputCls} value={p.nabhNumber || ""} onChange={set("nabhNumber")} />
        </Field>
        <Field label="GST Number">
          <input className={inputCls} value={p.gstNumber || ""} onChange={set("gstNumber")} />
        </Field>
        <Field label="PAN Number">
          <input className={inputCls} value={p.panNumber || ""} onChange={set("panNumber")} />
        </Field>
        <Field label="Website">
          <input
            className={inputCls}
        ...
        <Field label="Official email" required>
          <input type="email" className={inputCls} value={p.email || ""} onChange={set("email")} />
        </Field>
        <Field label="Official phone" required>
          <input className={inputCls} value={p.phone || ""} onChange={set("phone")} />
        </Field>
        ...
        <Field label="District" required>
          <input className={inputCls} value={p.district || ""} onChange={set("district")} />
        </Field>
        <Field label="City" required>
          <input className={inputCls} value={p.city || ""} onChange={set("city")} />
        </Field>
        <Field label="PIN code" required>
          <input className={inputCls} value={p.pincode || ""} onChange={set("pincode")} />
        </Field>
      </div>
      <h3 className="text-sm font-semibold mt-8 mb-3">Hospital administrator</h3>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Field label="Name" required>
          <input className={inputCls} value={p.adminName || ""} onChange={set("adminName")} />
        </Field>
        ...
        <Field label="Phone" required>
          <input className={inputCls} value={p.adminPhone || ""} onChange={set("adminPhone")} />
        </Field>
      </div>
    </>
  );
...
function FacilityStep({ app, updateField }) {
  ...
  const num = (k) => (e) =>
    updateField("facility", k, e.target.value === "" ? "" : Number(e.target.value));
  ...
  return (
    <>
      <StepHeader
        ...
        subtitle="Capacity, staff, and services."
      />
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {numFields.map(([k, l]) => (
          <Field
            key={k}
            label={l}
            required={["totalBeds", "icuBeds", "doctors", "nurses"].includes(k)}
          >
            <input
              type="number"
              min={0}
...
function DocumentsStep({ app, update }) {
  const onPick = async (key, file) => {
    ...
    if (file.size > MAX_FILE) {
      alert("Max 10 MB per file.");
      return;
    }
    if (!/^(application\/pdf|image\/(jpe?g|png))$/.test(file.type)) {
      alert("PDF, JPG or PNG only.");
      return;
    }
      ...
      <div className="space-y-2">
        {REQUIRED_DOCS.map((d) => {
          ...
          return (
            <div
              key={d.key}
              className={`rounded-lg border p-3 flex items-center gap-3 ${file ? "border-success/40 bg-success-soft/30" : "border-border bg-card"}`}
            >
              <FileText className={`h-5 w-5 ${file ? "text-success" : "text-muted-foreground"}`} />
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium">
                  {d.label} {d.required && <span className="text-emergency">*</span>}
                </div>
                {file && (
              ...
              {file ? (
                <button
                  ...
                  className="text-xs text-emergency hover:underline"
                >
                  Remove
                </button>
              ) : (
  ...
  return (
    <>
      <StepHeader
        ...
        subtitle={`JPG/PNG • Max 10MB each • Minimum 10 photos • You have ${total}`}
      />
      <PhotoGrid
        title="Required"
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {labels.map((label) => {
        ...
        return (
          <div
            key={label}
            className={`rounded-lg border p-3 ${arr.length > 0 ? "border-success/40 bg-success-soft/30" : "border-border bg-card"}`}
          >
            <div className="flex items-center justify-between mb-2">
              <div className="text-sm font-medium">
                {label} {required && arr.length === 0 && <span className="text-emergency">*</span>}
              </div>
              <label className="text-xs font-semibold text-primary hover:underline cursor-pointer inline-flex items-center gap-1">
                ...
                <input
                  ...
                  className="hidden"
                  accept=".jpg,.jpeg,.png"
                  onChange={(e) => onPick(label, e.target.files || [])}
                />
              </label>
            </div>
            {arr.length > 0 ? (
              <div className="grid grid-cols-3 gap-1.5">
                {arr.map((p, i) => (
                  <div
                    key={i}
                    className="relative group aspect-square rounded overflow-hidden bg-secondary"
                  >
                    <img
                      src={p.dataUrl}
                      alt=""
...
function LocationStep({ app, updateField }) {
  ...
  const mapLink =
    l.latitude && l.longitude ? `https://www.google.com/maps?q=${l.latitude},${l.longitude}` : null;
  return (
    <>
      <StepHeader
        icon={MapPin}
        title="Location"
        subtitle="Pin your hospital so patients can find you."
      />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Latitude" required>
          <input className={inputCls} value={l.latitude || ""} onChange={set("latitude")} />
        </Field>
        <Field label="Longitude" required>
          <input className={inputCls} value={l.longitude || ""} onChange={set("longitude")} />
        </Field>
      </div>
      <div className="mt-4 flex flex-wrap gap-2">
        <button
          onClick={useMe}
          className="text-xs font-semibold rounded-md border border-border bg-card px-3 py-2 hover:bg-secondary"
        >
        ...
        {mapLink && (
          <a
            ...
            rel="noreferrer"
            href={mapLink}
            className="text-xs font-semibold rounded-md bg-primary-soft text-primary px-3 py-2 hover:bg-primary-soft/70"
          >
            Open in Google Maps ↗
          </a>
        )}
  ...
  return (
    <>
      <StepHeader
        ...
        subtitle="We sent codes to your official email and phone."
      />
...
function OtpBlock({ icon: Icon, label, target, scope, verified, onVerified }) {
  ...
  const send = () => {
    if (!target) {
      setError(`No ${label.toLowerCase()} on file`);
      return;
    }
    setBusy(true);
    setTimeout(() => {
  ...
  return (
    <div
      className={`rounded-lg border p-4 ${verified ? "border-success/40 bg-success-soft/30" : "border-border bg-card"}`}
    >
      <div className="flex items-center gap-3">
        <div
          className={`h-9 w-9 rounded-lg flex items-center justify-center ${verified ? "bg-success text-success-foreground" : "bg-primary-soft text-primary"}`}
        >
        ...
        {verified ? (
          ...
          <button
            onClick={send}
            disabled={busy}
            className="text-xs font-semibold rounded-md bg-primary text-primary-foreground px-3 py-1.5 hover:bg-primary-dark disabled:opacity-50"
          >
      ...
      {sent && !verified && (
        <div className="mt-3 space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={check}
              className="text-xs font-semibold rounded-md bg-primary text-primary-foreground px-4 hover:bg-primary-dark"
            >
              Verify
            </button>
            <button onClick={send} className="text-xs text-primary hover:underline">
              Resend
            </button>
          </div>
  ...
  return (
    <>
      <StepHeader
        icon={CheckCircle2}
        title="Review & submit"
        subtitle="Make sure everything looks right before sending to our review team."
      />
      <dl className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
        ...
        <Row
          k="Beds (Total / ICU)"
          v={`${app.facility.totalBeds || 0} / ${app.facility.icuBeds || 0}`}
        />
        <Row
          k="Doctors / Nurses"
          v={`${app.facility.doctors || 0} / ${app.facility.nurses || 0}`}
        />
        <Row k="Documents uploaded" v={`${docCount} files`} />
        <Row k="Photos uploaded" v={`${photoCount} photos`} />
        <Row
          k="Location"
          v={app.location.latitude ? `${app.location.latitude}, ${app.location.longitude}` : "—"}
        />
      ...
      <div className="mt-6 rounded-lg border border-primary/30 bg-primary-soft p-4 text-sm text-primary">
        On submit, your application enters{" "}
        <span className="font-bold">Pending facility review</span>. Our team will email you the
        outcome.
      </div>
    </>
  );
