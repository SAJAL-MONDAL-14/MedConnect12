...
const TABS = ["Bookings", "Health Records", "SOS Contacts", "Settings"];
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    document.title = "My profile — MedConnect";
  }, []);
          ...
          <div className="flex overflow-x-auto">
            {TABS.map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={`shrink-0 px-4 py-3 text-sm font-medium border-b-2 transition ${tab === t ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}
              >
                {t}
              </button>
            ))}
        ...
        <div className="py-5 space-y-5">
                  ...
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {myBookings[0].specialty} · {myBookings[0].hospital}
                      </div>
                    </div>
                    <span className="rounded-full bg-success-soft text-success px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider">
                      Confirmed
                    </span>
                  </div>
                  <div className="text-sm text-primary font-medium">
                    {myBookings[0].date} · {myBookings[0].time}
                  </div>
                  <div className="mt-3 flex items-center gap-2">
                    <button className="text-xs rounded-md border border-border px-3 py-1.5 hover:border-emergency hover:text-emergency">
                      Cancel
                    </button>
                  ...
                  {myBookings.slice(1).map((b) => (
                    <div key={b.id} className="rounded-xl bg-card border border-border p-4">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className="rounded-full bg-muted text-muted-foreground px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider">
                          Completed
                        </span>
                      </div>
                      <button className="mt-3 text-xs rounded-md border border-primary text-primary px-3 py-1.5 hover:bg-primary-soft">
                        Book again
                      </button>
                    </div>
                  ))}
              ...
              {[
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">
                      <span className="rounded bg-accent px-1.5 py-0.5 mr-1.5 text-[10px]">
                        {r.type}
                      </span>
                      {r.date}
                    </div>
            ...
            <>
              ...
              {sosContacts.map((c) => (
                <div
                  ...
                  className="rounded-xl bg-card border border-border p-4 flex items-center gap-3"
                >
                  <div className="h-9 w-9 rounded-full bg-emergency text-emergency-foreground flex items-center justify-center font-bold text-sm">
                    {c.id}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm">{c.name}</div>
                    <div className="text-xs text-muted-foreground">
                      {c.relation} · <span className="font-mono">{c.phone}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <MessageCircle
                      className={`h-4 w-4 ${c.whatsapp ? "text-success" : "text-text-muted"}`}
                    />
                    <Toggle initial={c.whatsapp} />
                    <button className="text-text-muted hover:text-emergency p-1">
          ...
          {tab === "Settings" && (
            <>
              <div className="rounded-xl bg-card border border-border p-4">
                ...
                <div className="inline-flex rounded-full border border-border bg-secondary p-0.5">
                  {["EN", "हि", "বা"].map((l, i) => (
                    <button
                      key={l}
                      className={`px-4 py-1.5 rounded-full text-xs font-medium ${i === 0 ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
                    >
                      {l}
                    </button>
                  ))}
              ...
              <Link
                to="/login"
                className="block w-full text-center rounded-md border-2 border-emergency text-emergency py-2.5 text-sm font-bold hover:bg-emergency-soft transition inline-flex items-center justify-center gap-2"
              >
