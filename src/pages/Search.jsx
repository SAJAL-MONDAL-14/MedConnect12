...
export default function SearchPage() {
  ...
  const [chips, setChips] = useState(
    searchFilters
      ? searchFilters.split(",").filter(Boolean)
  ...
  const results = useMemo(() => {
    ...
    switch (sort) {
      ...
      case "Bed availability":
        list.sort((a, b) => b.beds.general + b.beds.icu - (a.beds.general + a.beds.icu));
        break;
      case "Drive time":
        list.sort((a, b) => a.driveMin - b.driveMin);
        break;
      default:
        ...
        <div className="max-w-[1400px] mx-auto px-6 py-3 space-y-3">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-md border px-3 py-2 text-sm font-medium transition ${showFilters ? "bg-primary text-primary-foreground border-primary" : "border-border bg-card hover:border-primary"}`}
            >
          ...
          {chips.length > 0 && (
            <div className="flex items-center gap-2 flex-wrap">
              {chips.map((f) => (
                <span
                  key={f}
                  className="inline-flex items-center gap-1.5 rounded-full bg-primary-soft text-primary px-2.5 py-1 text-xs font-medium"
                >
                  {f}
                  <button
                    onClick={() => setChips((c) => c.filter((x) => x !== f))}
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                >
                  Sort: <span className="text-foreground font-medium">{sort}</span>{" "}
                  ...
                  <div className="absolute right-0 top-full mt-1 w-44 rounded-md bg-card border border-border shadow-elevated z-20 py-1">
                    {SORTS.map((s) => (
                      <button
                        ...
                        className={`w-full text-left px-3 py-1.5 text-sm hover:bg-secondary ${sort === s ? "text-primary font-medium" : "text-foreground"}`}
                      >
                        {s}
                      </button>
                    ))}
              ...
              <div className="inline-flex rounded-md border border-border bg-card overflow-hidden text-xs">
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 transition ${view === "list" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted"}`}
                >
                  <List className="h-3.5 w-3.5" /> List
                </button>
                <button
                  onClick={() => setView("map")}
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 transition ${view === "map" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted"}`}
                >
              ...
              {results.length === 0 ? (
                <div className="rounded-xl bg-card border border-dashed border-border p-10 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Try removing chips or changing your search.
                  </p>
                  <button
                    onClick={() => {
                      setQuery("");
                    ...
                    className="mt-4 rounded-md bg-primary text-primary-foreground px-4 py-2 text-xs font-semibold"
                  >
                    Clear search
                  </button>
                </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-[10px] uppercase tracking-wider rounded bg-accent text-accent-foreground px-1.5 py-0.5 font-semibold">
                          Doctor
                        </span>
                      </div>
                      <p className="text-xs text-muted-foreground">
                        {doctors[0].specialty} · {doctors[0].experience} yrs · North Bengal MC
                      </p>
                      <div className="flex items-center gap-3 mt-2">
                        <StarRating rating={doctors[0].rating} reviews={142} />
                        <span className="text-xs text-success inline-flex items-center gap-1">
                          <Clock className="h-3 w-3" /> {doctors[0].nextSlot}
                        </span>
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      <Link
                        to="/booking"
                        className="mt-2 inline-block rounded-md bg-primary text-primary-foreground px-3 py-1.5 text-xs font-semibold hover:bg-primary-dark"
                      >
                        Book
                      </Link>
                    </div>
          ...
          {SPECS.map((s) => {
            ...
            return (
              <button
                ...
                className={`rounded-full px-3 py-1 text-xs border transition ${on ? "bg-primary text-primary-foreground border-primary" : "bg-card border-border hover:border-primary"}`}
              >
                {s}
              </button>
            );
        ...
        {["Has beds", "Rating 4+", "Government", "Private", "Open now"].map((t) => {
          ...
          return (
            <button
              ...
              className={`rounded-full px-3 py-1 text-xs border transition ${on ? "bg-success text-success-foreground border-success" : "bg-card border-border hover:border-success"}`}
            >
              {t}
            </button>
          );
