      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          ...
          <div className="text-[11px] text-muted-foreground mt-1 font-mono">
            ...
            <span
              className={free === 0 ? "text-emergency font-bold" : "text-success font-semibold"}
            >
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <div className={`text-3xl font-bold font-mono w-10 text-center text-${color}`}>
            {free}
          </div>
          <button
...
function ToggleRow({ label, desc, on, setOn }) {
  return (
    <div className="flex items-center justify-between p-4 rounded-xl bg-card border border-border">
      ...
      >
        <span
          className={`absolute top-0.5 h-6 w-6 rounded-full bg-card shadow transition-all ${on ? "left-[22px]" : "left-0.5"}`}
        />
      </button>
    </div>
  );
...
export default function WardDashboard() {
  ...
  useEffect(() => {
    document.title = "Ward & ICU — MedConnect";
  }, []);

  const save = () => setSaved("just now");

  const updateWard = (id, occ) => {
    setWards((w) => w.map((x) => (x.id === id ? { ...x, occupied: occ } : x)));
    save();
  };
  const updateIcu = (id, occ) => {
    setIcus((i) => i.map((x) => (x.id === id ? { ...x, occupied: occ } : x)));
    save();
  };
  ...
  return (
    <div className="min-h-screen bg-background flex">
      {/* Sidebar */}
      <aside
        className="hidden md:flex flex-col w-[210px] shrink-0 text-white bg-gradient-to-b from-warning-soft/0 to-warning-soft/0"
        style={{ background: "linear-gradient(180deg, #7a4f1a 0%, #ba7517 100%)" }}
      >
        ...
        <div className="mx-3 mb-3 space-y-2">
          ...
          <div className="rounded-lg bg-white/10 border border-white/15 p-3">
            ...
            <div className={`text-xl font-bold mt-0.5 ${totalIcuFree === 0 ? "text-red-300" : ""}`}>
              {totalIcuFree}
            </div>
          </div>
        </div>
        ...
        <header className="bg-card border-b border-border h-16 flex items-center justify-between px-6 sticky top-0 z-30">
          <div>
            <div className="font-semibold">
              {view === "Dashboard" ? "Good morning, Ravi 👋" : view}
            </div>
            <div className="text-xs text-muted-foreground">
              Ward Staff · North Bengal Medical College
            </div>
          </div>
        ...
        <div className="bg-success-soft border-b border-success/20">
          <div className="max-w-full px-6 py-2 flex items-center justify-between text-xs">
            <span className="text-success inline-flex items-center gap-1.5">
              <CheckCircle2 className="h-3.5 w-3.5" /> Patients see your updates in real-time · Last
              sync: {savedAt}
            </span>
            <button
              onClick={save}
            ...
            {[
              {
                ...
                value: totalGenFree,
                total: wards.reduce((s, w) => s + w.total, 0),
                tone: totalGenFree <= 5 ? "emergency" : "success",
                icon: BedDouble,
              },
              {
                label: "ICU beds free",
                value: totalIcuFree,
                total: icus.reduce((s, i) => s + i.total, 0),
                tone: totalIcuFree === 0 ? "emergency" : totalIcuFree <= 2 ? "warning" : "success",
                icon: Activity,
              },
              {
                label: "OT status",
                value: ot ? "Open" : "Closed",
                total: null,
                tone: ot ? "success" : "emergency",
                icon: Stethoscope,
              },
              {
                label: "Ambulance",
                value: amb ? "Ready" : "Busy",
                total: null,
                tone: amb ? "success" : "warning",
                icon: Ambulance,
              },
            ].map((s) => (
              <div
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Use +/− to update occupied count. Changes broadcast instantly.
                </p>
              </div>
              <span className="text-xs text-success inline-flex items-center gap-1.5">
                <CheckCircle2 className="h-3.5 w-3.5" /> Saved {savedAt}
              </span>
            </div>
          ...
          <div className="rounded-xl bg-card border border-border p-6">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Critical care units — update immediately when a patient is admitted or discharged.
                </p>
              </div>
            </div>
            <div className="space-y-3">
