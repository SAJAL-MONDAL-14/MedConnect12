...
export default function OPDDashboard() {
  ...
  useEffect(() => {
    document.title = "OPD Reception — MedConnect";
  }, []);
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      <aside className="hidden md:flex flex-col w-[210px] shrink-0 text-white bg-gradient-to-b from-primary-dark to-primary">
        ...
        <div className="mx-3 mb-3 rounded-lg bg-white/10 border border-white/15 p-3">
          ...
          <div className="text-[10px] text-white/60 font-mono">
            {nextPatient?.time} · Token #{nextPatient?.token}
          </div>
        </div>
        <Link
          to="/staff/login"
        ...
        <header className="bg-card border-b border-border h-16 flex items-center justify-between px-6 sticky top-0 z-30">
          <div>
            <div className="font-semibold">
              {view === "Dashboard" ? "Good morning, Pooja 👋" : view}
            </div>
            <div className="text-xs text-muted-foreground">
              OPD · North Bengal Medical College ·{" "}
            ...
            <div className="px-6 py-4 border-b border-border flex items-center justify-between flex-wrap gap-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  Click a row to check-in or update status
                </p>
              </div>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {queue.map((p) => (
                    <tr
                      key={p.id}
                      className={`hover:bg-secondary/50 ${p.status === "With Doctor" ? "bg-primary-soft/20" : ""}`}
                    >
                      ...
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span
                          className={`text-[10px] font-semibold rounded-full px-2 py-0.5 uppercase ${p.type === "Walk-in" ? "bg-accent text-accent-foreground" : "bg-primary-soft text-primary"}`}
                        >
                          {p.type}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        ...
                        {!p.paid && (
                          <div className="text-[10px] text-warning font-medium">Unpaid</div>
                        )}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          <button
                            onClick={() => markStatus(p.id, "With Doctor")}
                          ...
                          <button
                            onClick={() => markStatus(p.id, "No-show")}
                            title="No-show"
      ...
      {walkinModal && (
        ...
        >
          <div
            className="bg-card border border-border rounded-2xl w-full max-w-md p-6 shadow-elevated"
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    value={wDoc}
                    onChange={(e) => setWDoc(e.target.value)}
                    className="w-full rounded-md bg-input border border-border px-3 py-2 text-sm outline-none focus:border-primary"
                  >
                    {DOCTORS.map((d) => (
