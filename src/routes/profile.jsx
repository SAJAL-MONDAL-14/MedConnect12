      ...
      <div className="max-w-[700px] mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="sticky top-16 bg-background z-20 border-b border-border mt-4 -mx-4 sm:-mx-6 px-4 sm:px-6">
          <div className="flex overflow-x-auto">
            {TABS.map((t) => (
              <button key={t} onClick={() => setTab(t)} className={`shrink-0 px-4 py-3 text-sm font-medium border-b-2 transition ${tab === t ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}>{t}</button>
            ))}
          </div>
        </div>
        ...
        <div className="py-5 space-y-5">
          ...
          {tab === "Health Records" && (
            <>
              <div className="flex items-center justify-between">
                ...
                <button className="inline-flex items-center gap-1 rounded-md bg-primary text-primary-foreground px-3 py-1.5 text-xs font-semibold hover:bg-primary-dark"><Upload className="h-3.5 w-3.5" /> Upload</button>
              </div>
              {[
                ...
                <div key={r.name} className="rounded-xl bg-card border border-border p-4 flex items-center gap-3">
                  <div className="h-10 w-10 rounded-md bg-primary-soft text-primary flex items-center justify-center"><FileText className="h-5 w-5" strokeWidth={1.5} /></div>
                  <div className="flex-1 min-w-0">
          ...
          {tab === "SOS Contacts" && (
            <>
              <div className="rounded-md bg-emergency-soft border border-emergency/20 px-4 py-3 text-xs text-emergency">These contacts are notified via WhatsApp when you trigger SOS.</div>
              {sosContacts.map((c) => (
