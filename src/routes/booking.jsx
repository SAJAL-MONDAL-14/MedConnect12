        ...
        <div className="flex items-center gap-2 mb-6">
          {[1, 2].map((n, i) => (
            <div key={n} className="flex items-center gap-2 flex-1">
              <div className={`h-7 w-7 rounded-full flex items-center justify-center text-xs font-bold ${step >= n ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
                {step > n ? <CheckCircle2 className="h-4 w-4" /> : n}
              </div>
              <span className={`text-xs font-medium ${step >= n ? "text-foreground" : "text-muted-foreground"}`}>{n === 1 ? "Patient details" : "Payment"}</span>
              ...
              <div className="rounded-xl bg-card border border-border p-5">
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={3} placeholder="e.g. Chest pain for 2 days, mild shortness of breath..." className="w-full rounded-md bg-input border border-border px-3 py-2 text-sm outline-none focus:border-primary resize-none" />
                  </div>
                </div>
                <button disabled={!valid} onClick={() => setStep(2)} className="mt-5 w-full rounded-md bg-primary text-primary-foreground py-2.5 text-sm font-semibold hover:bg-primary-dark transition disabled:opacity-50">Continue to payment</button>
              </div>
            )}
...
function Field({ label, value, onChange, mono }) {
  return (
    <div>
      ...
      <input value={value} onChange={(e) => onChange(e.target.value)} className={`w-full rounded-md bg-input border border-border px-3 py-2 text-sm outline-none focus:border-primary ${mono ? "font-mono" : ""}`} />
    </div>
  );
}
...
function PayOption({ selected, onClick, icon, title, desc }) {
  return (
    <button onClick={onClick} className={`w-full text-left rounded-lg border-2 p-4 flex items-center gap-3 transition ${selected ? "border-primary bg-primary-soft" : "border-border bg-card hover:border-primary/50"}`}>
      <div className={`h-10 w-10 rounded-md flex items-center justify-center ${selected ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>{icon}</div>
