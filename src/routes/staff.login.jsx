...
import { Mail, Lock, Building2, BedDouble, CreditCard, ClipboardList, ShieldCheck, ChevronRight } from "lucide-react";
...
export default function StaffLogin() {
  ...
  useEffect(() => { document.title = "Staff login — MedConnect"; }, []);
    ...
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <div className="hidden lg:flex flex-col justify-between p-10 text-white relative overflow-hidden" style={{ background: "var(--gradient-hero)" }}>
        ...
        <div className="relative">
          ...
          <p className="mt-4 text-white/70 max-w-md">Each role sees only what they need — no clutter, just the right tools.</p>
          <div className="mt-8 grid grid-cols-3 gap-3 max-w-md">
            {[{ v: "48", l: "Hospitals" }, { v: "1.2k", l: "Bookings/day" }, { v: "<2s", l: "Update lag" }].map((s) => (
        ...
        <div className="w-full max-w-sm">
          ...
          {!showForm ? (
            <>
              ...
              <p className="text-sm text-muted-foreground mt-1 mb-6">Select your role to continue.</p>
              <div className="space-y-2 mb-5">
                {ROLES.map((r) => (
                  <button key={r.id} onClick={() => setSelectedRole(r.id)}
                    ...
                    <div className={`h-10 w-10 rounded-lg flex items-center justify-center shrink-0 transition ${
                      selectedRole === r.id ? `bg-${r.color} text-${r.color}-foreground` : "bg-secondary text-muted-foreground"
                    }`}>
                    ...
                    {selectedRole === r.id && <ChevronRight className={`h-4 w-4 text-${r.color} shrink-0`} />}
                  </button>
                ))}
              </div>
              <button disabled={!selectedRole} onClick={() => setShowForm(true)}
              ...
              <div className="mt-4 flex justify-between text-xs">
                ...
                <Link to="/hospital-doctor/login" className="text-primary hover:underline">Hospital doctor</Link>
              </div>
            </>
          ) : (
            <>
              <button onClick={() => setShowForm(false)} className="text-xs text-muted-foreground hover:text-foreground mb-4 inline-flex items-center gap-1">
                ← Change role
              </button>
              {role && (
                <div className={`mb-5 rounded-lg bg-${role.color}-soft border border-${role.color}/20 px-3 py-2 text-xs font-semibold text-${role.color}`}>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <div className="flex items-center gap-2 rounded-md border border-border bg-input px-3 focus-within:border-primary">
                    ...
                    <input type="email" defaultValue="staff@nbmc.in" className="flex-1 bg-transparent py-2.5 text-sm outline-none" />
                  </div>
                </div>
                <div>
              ...
              <button onClick={() => role && navigate(role.route)}
