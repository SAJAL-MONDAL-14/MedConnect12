        ...
        {step === 1 ? (
          <>
            ...
            <div className="flex items-stretch rounded-md border border-border overflow-hidden focus-within:border-primary transition">
              ...
              <input type="tel" placeholder="98765 43210" className="flex-1 bg-input px-3 py-2.5 text-sm outline-none font-mono" maxLength={10} />
            </div>
            <button onClick={() => setStep(2)} className="mt-5 w-full rounded-md bg-primary text-primary-foreground py-2.5 text-sm font-semibold hover:bg-primary-dark transition">Send OTP</button>
            <p className="mt-4 text-xs text-center text-muted-foreground">Don't have an account? <a className="text-primary font-semibold hover:underline cursor-pointer">Register</a></p>
            <div className="mt-6 flex justify-center">
              <div className="inline-flex rounded-full border border-border p-0.5 bg-secondary">
                {["EN", "हि", "বা"].map((l, i) => (
                  <button key={l} className={`px-3 py-1 rounded-full text-xs font-medium ${i === 0 ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}>{l}</button>
                ))}
              </div>
            </div>
          </>
              ...
              {otp.map((v, i) => (
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setOtp(next);
                    if (e.target.value && i < 5) e.target.nextElementSibling?.focus();
                  }}
                  className={`h-14 w-12 text-center font-mono text-lg font-semibold rounded-md border bg-card outline-none transition ${v ? "border-primary text-primary" : "border-border focus:border-primary"}`}
                  maxLength={1}
                />
              ))}
            </div>
            <p className="mt-4 text-xs text-text-muted text-center font-mono">Resend OTP in 0:28</p>
            <Link to="/" className="mt-5 block text-center rounded-md bg-primary text-primary-foreground py-2.5 text-sm font-semibold hover:bg-primary-dark transition">Verify</Link>
          </>
        )}
      </div>
      {/* All portal links */}
