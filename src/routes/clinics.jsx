...
export default function ClinicsPage() {
  ...
  useEffect(() => {
    document.title = "Individual Doctor Clinics — MedConnect";
  }, []);

  const list = useMemo(() => {
    return clinicDoctors.filter((d) => {
      const matchQ = !q.trim() || d.name.toLowerCase().includes(q.toLowerCase()) || d.specialty.toLowerCase().includes(q.toLowerCase()) || d.clinicName.toLowerCase().includes(q.toLowerCase());
        ...
        <div className="relative max-w-[1200px] mx-auto px-6 py-14 text-white">
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-3 max-w-2xl">
            <div className="relative flex-1">
              ...
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by doctor, specialty or clinic name" className="w-full pl-10 pr-4 py-3 rounded-xl bg-white text-foreground border border-white/30 outline-none text-sm shadow-lg" />
            </div>
      ...
      <section className="max-w-[1200px] mx-auto px-6 py-6">
        ...
        {list.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {list.map((d) => {
              const bg = d.avatarColor === "primary" ? "bg-primary" : d.avatarColor === "success" ? "bg-success" : d.avatarColor === "warning" ? "bg-warning" : "bg-emergency";
              return (
                <div key={d.id} className="group bg-card border border-border rounded-2xl p-5 hover:border-primary hover:shadow-xl hover:shadow-primary/5 transition">
                  <div className="flex items-start gap-4">
                    <div className={`h-14 w-14 rounded-full ${bg} text-white flex items-center justify-center text-lg font-bold shrink-0`}>{d.initials}</div>
                  ...
                  <div className="mt-3 p-3 rounded-lg bg-secondary/50">
                    <div className="flex items-center gap-1.5 text-xs font-medium text-foreground"><Building className="h-3.5 w-3.5 text-primary" /> {d.clinicName}</div>
