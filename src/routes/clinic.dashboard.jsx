...
export default function ClinicDashboard() {
  ...
  useEffect(() => {
    document.title = "Clinic dashboard — MedConnect";
  }, []);
    ...
    <div className="min-h-screen bg-background flex">
      <aside className="hidden md:flex flex-col w-[220px] shrink-0 text-white bg-gradient-to-b from-primary via-primary-dark to-foreground">
        ...
        <nav className="flex-1 py-3">
          {navItems.map((n) => (
            <button key={n.label} onClick={() => setView(n.label)} className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-xs font-medium border-l-2 transition text-left ${view === n.label ? "bg-white/10 border-white text-white" : "border-transparent text-white/70 hover:bg-white/5"}`}>
              <n.icon className="h-4 w-4" strokeWidth={1.5} /> {n.label}
            </button>
          ))}
        </nav>
        <Link to="/login" className="m-4 inline-flex items-center gap-2 text-xs text-white/70 hover:text-white"><LogOut className="h-4 w-4" /> Logout</Link>
      </aside>
    ...
    <>
      <div className="rounded-2xl p-6 text-white bg-gradient-to-r from-primary to-primary-dark relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <button className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-white text-primary text-sm font-semibold hover:bg-white/90 transition"><Eye className="h-4 w-4" /> View today's schedule</button>
        </div>
      </div>
...
function AppointmentsView({ compact = false }) {
  ...
  const update = (id, status) => setAppts((a) => a.map((x) => x.id === id ? { ...x, status } : x));
  return (
...
function RecentPatients() {
  ...
  return (
    <div className="rounded-xl bg-card border border-border p-5">
      <div className="flex items-center justify-between mb-4"><h2 className="font-semibold">Recent patients</h2><button className="text-xs text-primary hover:underline">All patients</button></div>
      <div className="space-y-3">
        {patients.map((p) => (
          <div key={p.name} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-secondary transition">
            <div className="h-9 w-9 rounded-full bg-primary-soft text-primary flex items-center justify-center text-xs font-bold">{p.name.split(" ").map(n => n[0]).join("")}</div>
            <div className="flex-1 min-w-0"><div className="text-sm font-medium truncate">{p.name}</div><div className="text-[11px] text-muted-foreground">Age {p.age} · {p.visits} visits</div></div>
  ...
  return (
    <div className="rounded-xl bg-card border border-border p-6">
      <div className="flex items-center justify-between mb-5">
        <div><h2 className="font-semibold text-lg">Weekly schedule</h2><p className="text-xs text-muted-foreground">Tap a slot to toggle availability</p></div>
          ...
          {slots.map(s => (
            <Fragment key={s}>
              ...
              {days.map((d, i) => {
                ...
                return (
                  <button key={d + s} className={`h-10 rounded-md text-[11px] font-medium border transition ${off ? "bg-muted text-muted-foreground border-border" : "bg-success-soft text-success border-success/20 hover:bg-success hover:text-success-foreground"}`}>{off ? "Off" : "Open"}</button>
                );
              })}
            </Fragment>
          ))}
...
function EarningsView() {
  ...
  return (
    <div className="grid lg:grid-cols-[1.4fr_1fr] gap-4">
      <div className="rounded-xl bg-card border border-border p-5">
        <div className="flex items-center justify-between mb-5"><h2 className="font-semibold">Earnings this week</h2><span className="text-xs text-success font-semibold">+22% vs last week</span></div>
      ...
      <div className="rounded-xl bg-card border border-border p-5 space-y-3">
        ...
        {[{ label: "Today", value: "₹8,400", delta: "+22%" }, { label: "This week", value: "₹52,200", delta: "+18%" }, { label: "This month", value: "₹2.1L", delta: "+24%" }, { label: "Pending payout", value: "₹14,800", delta: "Friday" }].map(r => (
          <div key={r.label} className="flex items-center justify-between rounded-lg bg-secondary px-4 py-3">
            <div><div className="text-xs text-muted-foreground">{r.label}</div><div className="text-lg font-bold font-mono">{r.value}</div></div>
        ...
        <div className="mt-5 space-y-2">
          {[5, 4, 3, 2, 1].map(n => (
            <div key={n} className="flex items-center gap-2 text-xs">
              ...
              <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden"><div className="h-full bg-warning" style={{ width: `${[80, 15, 3, 1, 1][5 - n]}%` }} /></div>
        ...
        {reviews.map((r, i) => (
          <div key={i} className="border-b border-border last:border-0 pb-4 last:pb-0">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-0.5">{Array.from({ length: 5 }).map((_, i) => <Star key={i} className={`h-3.5 w-3.5 ${i < r.rating ? "fill-warning text-warning" : "text-muted"}`} />)}</div>
            </div>
