...
function TabBtn({ active, onClick, icon: Icon, label, count }) {
  return (
    <button onClick={onClick} className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium border-b-2 whitespace-nowrap transition ${active ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}>
      ...
      {tab === "services" && (
        <div className="grid sm:grid-cols-2 gap-3">
          {services.map((s) => {
            ...
            return (
              <Wrap key={s.label} {...wrapProps} className={`flex items-center gap-3 rounded-lg border border-border bg-card p-4 transition ${s.on ? "hover:border-primary hover:shadow-card cursor-pointer" : "opacity-70"}`}>
                <div className={`h-10 w-10 rounded-lg flex items-center justify-center ${s.on ? "bg-success-soft text-success" : "bg-muted text-text-muted"}`}><s.icon className="h-5 w-5" /></div>
                <div className="flex-1">
                  <div className="text-sm font-medium">{s.label}</div>
                  <div className="text-[11px] text-muted-foreground">{s.on ? "Available at this facility" : "Not available"}</div>
                </div>
                {s.on ? (
                  <span className="inline-flex items-center gap-1 text-xs font-semibold text-primary whitespace-nowrap">{cta} <ChevronRight className="h-3.5 w-3.5" /></span>
                ) : (
      ...
      {tab === "lab" && (
        <div className="space-y-3">
          <div className="rounded-xl border border-border bg-card p-4 flex items-center justify-between">
            ...
            <Link to="/labs" className="text-sm font-medium text-primary hover:underline">View all →</Link>
          </div>
