            ...
            <div className="space-y-2">
              {labs.map((l) => (
                <button
                  ...
                  className={`w-full text-left rounded-lg p-3 border transition ${selectedLab === l.id ? "bg-primary-soft border-primary text-primary" : "bg-card border-border hover:border-primary/40"}`}
                >
          ...
          <div className="rounded-xl bg-card border border-border p-4">
            ...
            <ul className="space-y-2 text-xs text-muted-foreground">
              <li className="flex gap-2">
                <Check className="h-3.5 w-3.5 text-success shrink-0 mt-0.5" /> NABL accredited only
              </li>
              <li className="flex gap-2">
                <Check className="h-3.5 w-3.5 text-success shrink-0 mt-0.5" /> Reports in 24 hrs
              </li>
              <li className="flex gap-2">
                <Check className="h-3.5 w-3.5 text-success shrink-0 mt-0.5" /> 10% off on 2+ tests
              </li>
            </ul>
          </div>
        </aside>
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            {categories.map((c) => (
              <button
                key={c}
                onClick={() => setCat(c)}
                className={`rounded-full px-3.5 py-1.5 text-xs font-medium border transition ${cat === c ? "bg-primary text-primary-foreground border-primary" : "bg-card text-muted-foreground border-border hover:text-foreground hover:border-primary/40"}`}
              >
                {c}
              </button>
            ))}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        {t.homeCollection && (
                          <span className="inline-flex items-center gap-1 rounded-full bg-success-soft text-success px-2 py-0.5 text-[10px] font-medium">
                        ...
                        {t.walkIn && (
                          <span className="inline-flex items-center gap-1 rounded-full bg-primary-soft text-primary px-2 py-0.5 text-[10px] font-medium">
                            <Building2 className="h-3 w-3" /> Walk-in
                          </span>
                        )}
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      ...
                      {qty === 0 ? (
                        <button
                          onClick={() => add(t.id)}
                          className="mt-2 inline-flex items-center gap-1 rounded-md bg-success text-success-foreground px-3 py-1.5 text-xs font-semibold hover:bg-success-dark transition"
                        >
                        ...
                        <div className="mt-2 inline-flex items-center gap-1 rounded-md border border-border bg-card">
                          <button
                            onClick={() => remove(t.id)}
                            className="h-7 w-7 inline-flex items-center justify-center hover:bg-secondary rounded-l-md"
                          >
                          ...
                          <button
                            onClick={() => add(t.id)}
                            className="h-7 w-7 inline-flex items-center justify-center hover:bg-secondary rounded-r-md"
                          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-sm font-semibold">
                    Cart · {cartItems.length} test{cartItems.length > 1 ? "s" : ""}
                  </div>
                  <div className="text-xs text-muted-foreground truncate max-w-[260px]">
                    {cartItems.map((c) => c.name.split(" ")[0]).join(" + ")}
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 bg-foreground/40 backdrop-blur flex items-center justify-center p-4">
          <div className="w-full max-w-md rounded-2xl bg-card border border-border shadow-elevated p-7 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {mode === "home"
                ? "Phlebotomist will arrive at your address in the selected slot."
                : "Walk in any time within your slot — no waiting."}
            </p>
            <div className="mt-5 rounded-xl bg-secondary border border-border p-4 text-left text-sm">
              <div className="flex justify-between mb-1.5">
                <span className="text-muted-foreground">Booking ID</span>
                <span className="font-mono font-semibold">
                  LAB-{Math.floor(Math.random() * 9000) + 1000}
                </span>
              </div>
              <div className="flex justify-between mb-1.5">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-mono font-bold text-success">₹{finalTotal}</span>
              </div>
            </div>
            <div className="mt-5 grid grid-cols-2 gap-2">
              <button
                ...
                className="rounded-md border border-border py-2.5 text-sm font-medium hover:bg-secondary"
              >
                Done
              </button>
              <Link
                to="/profile"
                className="rounded-md bg-primary text-primary-foreground py-2.5 text-sm font-semibold hover:bg-primary-dark text-center"
              >
                View bookings
              </Link>
            </div>
