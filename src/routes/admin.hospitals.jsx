...
import {
  Building2, Search, ChevronLeft, CheckCircle2, XCircle, AlertTriangle,
  Pause, FileText, Image as ImageIcon, MapPin, Mail, Phone, Download, X, Send,
} from "lucide-react";
import {
  listApplications, getApplication, updateStatus,
  STATUS, STATUS_META, getAudit, listEmails, sendMockEmail, EMAIL_TEMPLATES,
...
export default function AdminHospitals() {
  ...
  useEffect(() => { document.title = "Hospital applications — Admin"; refresh(); }, []);

  const filtered = useMemo(() => items.filter((a) => {
    if (filter !== "ALL" && a.status !== filter) return false;
    if (!q) return true;
    const s = q.toLowerCase();
    return [a.id, a.profile.hospitalName, a.profile.city, a.profile.email].filter(Boolean).some((v) => String(v).toLowerCase().includes(s));
  }), [items, q, filter]);
      ...
      <main className="max-w-7xl mx-auto px-6 py-6">
        ...
        <div className="mb-4 flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 flex-1 min-w-[200px] rounded-md border border-border bg-card px-3">
            ...
            <input className="flex-1 bg-transparent py-2 text-sm outline-none" placeholder="Search hospital, city, email, ID…" value={q} onChange={(e) => setQ(e.target.value)} />
          </div>
          <select value={filter} onChange={(e) => setFilter(e.target.value)} className="rounded-md border border-border bg-card px-3 py-2 text-sm">
            {FILTERS.map((s) => <option key={s} value={s}>{s === "ALL" ? "All statuses" : STATUS_META[s]?.label || s}</option>)}
          </select>
        </div>
...
function StatusBadge({ meta }) {
  ...
  return <span className={`inline-block text-[10px] font-semibold uppercase tracking-wide rounded px-2 py-0.5 bg-${tone}-soft text-${tone}`}>{meta.label}</span>;
}
...
function DetailDrawer({ id, onClose }) {
  ...
  const applyAction = () => {
    ...
    if (actionModal === "approve") {
      ...
      sendMockEmail({ to: a.profile.adminEmail, subject: t.subject, body: t.body, applicationId: a.id });
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end" onClick={onClose}>
      ...
      <aside onClick={(e) => e.stopPropagation()} className="relative w-full max-w-3xl h-full bg-background shadow-elevated flex flex-col">
        {/* Header */}
        <div className="border-b border-border p-5 flex items-start gap-3">
          <div className="h-10 w-10 rounded-lg bg-primary-soft text-primary flex items-center justify-center"><Building2 className="h-5 w-5" /></div>
        ...
        <div className="border-t border-border p-4 flex flex-wrap gap-2">
          <button onClick={() => setActionModal("approve")} className="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold rounded-md bg-success text-success-foreground hover:bg-success-dark"><CheckCircle2 className="h-4 w-4" /> Approve</button>
          <button onClick={() => setActionModal("info")}    className="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold rounded-md bg-warning text-warning-foreground"><AlertTriangle className="h-4 w-4" /> Request info</button>
          <button onClick={() => setActionModal("reject")}  className="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold rounded-md bg-emergency text-emergency-foreground"><XCircle className="h-4 w-4" /> Reject</button>
          <button onClick={() => setActionModal("suspend")} className="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold rounded-md border border-border bg-card"><Pause className="h-4 w-4" /> Suspend</button>
        </div>

        {actionModal && (
          <div className="absolute inset-0 z-50 bg-black/50 flex items-center justify-center p-4" onClick={() => setActionModal(null)}>
            <div onClick={(e) => e.stopPropagation()} className="w-full max-w-md rounded-xl bg-card border border-border p-5">
              <h3 className="text-base font-bold mb-1">
                {actionModal === "approve" ? "Approve application" :
                 actionModal === "reject"  ? "Reject application" :
                 actionModal === "info"    ? "Request more information" : "Suspend hospital"}
              </h3>
              <p className="text-xs text-muted-foreground mb-3">An email will be sent to <span className="font-mono">{app.profile.adminEmail}</span>.</p>
              {actionModal !== "approve" && (
                <textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={4} placeholder="Type message to hospital…"
              ...
              <div className="flex justify-end gap-2">
                <button onClick={() => setActionModal(null)} className="px-3 py-2 text-xs rounded-md border border-border bg-card">Cancel</button>
                <button onClick={applyAction} className="px-3 py-2 text-xs font-semibold rounded-md bg-primary text-primary-foreground inline-flex items-center gap-1.5">
        ...
        {zoom && (
          <div onClick={() => setZoom(null)} className="absolute inset-0 z-50 bg-black/80 flex items-center justify-center p-6">
...
function KV({ k, v }) {
  return <div className="rounded-md border border-border bg-card px-3 py-2 text-sm"><div className="text-[10px] uppercase tracking-wide text-muted-foreground">{k}</div><div className="font-medium truncate">{v || "—"}</div></div>;
}
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-2">
        ...
        <KV k="Email" v={<span className="inline-flex items-center gap-1">{p.email} {app.emailVerified && <CheckCircle2 className="h-3 w-3 text-success" />}</span>} />
        <KV k="Phone" v={<span className="inline-flex items-center gap-1">{p.phone} {app.phoneVerified && <CheckCircle2 className="h-3 w-3 text-success" />}</span>} />
        <KV k="Website" v={p.website ? <a href={p.website} target="_blank" rel="noreferrer" className="text-primary hover:underline">{p.website}</a> : "—"} />
...
function FacilityTab({ app }) {
  ...
  const num = [["totalBeds","Total beds"],["icuBeds","ICU"],["emergencyBeds","Emergency"],["ventilators","Ventilators"],["ambulances","Ambulances"],["operationTheatres","OT"],["doctors","Doctors"],["nurses","Nurses"],["supportStaff","Support staff"]];
...
function DocumentsTab({ app }) {
  ...
  if (entries.length === 0) return <div className="text-sm text-muted-foreground">No documents uploaded.</div>;
  return (
    <div className="space-y-2">
      {entries.map(([k, f]) => (
        <div key={k} className="rounded-lg border border-border bg-card p-3 flex items-center gap-3">
          <FileText className="h-5 w-5 text-primary" />
          <div className="flex-1 min-w-0">
            <div className="text-sm font-medium">{k}</div>
            <div className="text-xs text-muted-foreground truncate">{f.name} • {(f.size/1024).toFixed(0)} KB</div>
          </div>
          <a href={f.dataUrl} download={f.name} className="text-xs font-semibold text-primary inline-flex items-center gap-1 hover:underline"><Download className="h-3.5 w-3.5" /> Download</a>
        </div>
      ))}
    </div>
  );
...
function PhotosTab({ app, onZoom }) {
  ...
  if (entries.length === 0) return <div className="text-sm text-muted-foreground">No photos uploaded.</div>;
  return (
    <div className="space-y-4">
      {entries.map(([label, arr]) => (
        <div key={label}>
          <div className="text-xs font-semibold mb-1.5">{label} <span className="text-muted-foreground font-normal">({arr.length})</span></div>
          <div className="grid grid-cols-4 sm:grid-cols-6 gap-1.5">
            {arr.map((p, i) => (
              <img key={i} src={p.dataUrl} alt="" onClick={() => onZoom(p.dataUrl)}
...
function LocationTab({ app }) {
  ...
  if (!l.latitude) return <div className="text-sm text-muted-foreground">Location not provided.</div>;
  const link = `https://www.google.com/maps?q=${l.latitude},${l.longitude}`;
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2"><KV k="Latitude" v={l.latitude} /><KV k="Longitude" v={l.longitude} /></div>
      <a href={link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-sm font-semibold text-primary hover:underline"><MapPin className="h-4 w-4" /> Open in Google Maps ↗</a>
...
function AuditTab({ app }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-xs font-bold uppercase tracking-wide text-muted-foreground mb-2">Emails sent</h3>
        {emails.length === 0 ? <div className="text-xs text-muted-foreground">No emails sent yet.</div> : (
          <div className="space-y-2">
            {emails.map((e) => (
              <div key={e.id} className="rounded-lg border border-border bg-card p-3">
                <div className="flex items-center justify-between text-xs text-muted-foreground">
                  <span>→ {e.to}</span><span>{new Date(e.sentAt).toLocaleString()}</span>
                </div>
                <div className="text-sm font-semibold mt-1">{e.subject}</div>
                <pre className="text-xs text-muted-foreground whitespace-pre-wrap mt-1 font-sans">{e.body}</pre>
              </div>
            ))}
          </div>
        )}
      ...
      <div>
        <h3 className="text-xs font-bold uppercase tracking-wide text-muted-foreground mb-2">Audit log</h3>
