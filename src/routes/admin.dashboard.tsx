...
import type { ClinicApplication } from "./clinic.apply";
...
type View = "Dashboard" | "Analytics" | "Hospitals" | "Labs" | "Doctors" | "Clinic Applications" | "Patients" | "Staff" | "SOS Events" | "Bookings" | "Settings" | "Billing";
...
function ClinicApplicationsView() {
  const [apps, setApps] = useState<ClinicApplication[]>([]);
  ...
  const persist = (next: ClinicApplication[]) => {
    setApps(next);
    localStorage.setItem("clinic_applications", JSON.stringify(next));
  };

  const setStatus = (id: string, status: ClinicApplication["status"]) => {
...
function StatusBadge({ status }: { status: ClinicApplication["status"] }) {
  const map = {
