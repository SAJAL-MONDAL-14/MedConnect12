...
export default function StaffDashboard() {
  ...
  useEffect(() => {
    document.title = "Staff dashboard — MedConnect";
  }, []);
    ...
    <>
      <div className="rounded-2xl p-5 sm:p-6 text-white relative overflow-hidden" style={{ background: "var(--gradient-hero)" }}>
        ...
        <div className="relative flex items-start justify-between flex-wrap gap-4">
          <div>
            <div className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full bg-white/15 backdrop-blur border border-white/20"><span className="live-dot" /> Patients see your updates in real time</div>
      ...
      <div className="px-6 py-4 border-b border-border flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          {["All", "Pending", "Confirmed", "Completed"].map((f) => (
            <button key={f} onClick={() => setFilter(f)} className={`text-xs rounded-full px-3 py-1 border transition ${filter === f ? "bg-primary text-primary-foreground border-primary" : "border-border text-muted-foreground hover:border-primary"}`}>{f}</button>
          ))}
        </div>
      </div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            ...
            {filtered.length === 0 && (<tr><td colSpan={6} className="px-6 py-10 text-center text-sm text-muted-foreground">No appointments match this filter.</td></tr>)}
          </tbody>
        </table>
      </div>
    </div>
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl bg-card border border-border p-5 flex items-center gap-3 flex-wrap">
        ...
        <button onClick={() => setAdding((s) => !s)} className="rounded-md bg-primary text-primary-foreground px-3 py-2 text-xs font-semibold hover:bg-primary-dark inline-flex items-center gap-1"><Plus className="h-4 w-4" /> Add doctor</button>
      </div>
      {adding && (
...
function ReportsView() {
  const days = [{ d: "Mon", v: 60 }, { d: "Tue", v: 78 }, { d: "Wed", v: 52 }, { d: "Thu", v: 88 }, { d: "Fri", v: 95 }, { d: "Sat", v: 70 }, { d: "Sun", v: 100 }];
  return (
    <div className="grid lg:grid-cols-2 gap-4">
      <div className="rounded-xl bg-card border border-border p-5">
        <div className="flex items-center justify-between mb-4"><h2 className="font-semibold">Bookings this week</h2><span className="text-xs text-muted-foreground">Last 7 days</span></div>
      ...
      <div className="rounded-xl bg-card border border-border p-5 space-y-3">
        ...
        {[{ label: "Today", value: "₹21,000", delta: "+12%" }, { label: "This week", value: "₹1.2L", delta: "+8%" }, { label: "This month", value: "₹4.8L", delta: "+18%" }].map((r) => (
