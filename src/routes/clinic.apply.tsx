...
export const Route = createFileRoute("/clinic/apply")({
  head: () => ({ meta: [{ title: "Apply your clinic — MedConnect" }] }),
  component: ClinicApply,
});
...
function ClinicApply() {
  const navigate = useNavigate();
  const [step, setStep] = useState(1);
  const [submitted, setSubmitted] = useState(false);
  const [form, setForm] = useState<Omit<ClinicApplication, "id" | "submittedAt" | "status">>({
      ...
      <div className="min-h-screen bg-background flex items-center justify-center px-6">
        <div className="max-w-md w-full bg-card border border-border rounded-2xl p-8 text-center shadow-lg">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            <Link to="/" className="px-4 py-2.5 rounded-lg border border-border text-sm font-medium hover:border-primary hover:text-primary transition">
              Back home
            </Link>
            <Link to="/clinic/login" className="px-4 py-2.5 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:opacity-90">
              Go to clinic login
            </Link>
          </div>
        </div>
      ...
      <section className="bg-gradient-to-br from-primary-soft via-background to-background border-b border-border">
        <div className="max-w-[1100px] mx-auto px-6 py-10">
          ...
          <p className="mt-2 text-muted-foreground max-w-2xl">
            Run an independent practice? Reach thousands of patients, manage online bookings, and grow your clinic with our tools — completely free to get started.
          </p>
        </div>
      </section>
      ...
      <div className="max-w-[1100px] mx-auto px-6 py-8 grid lg:grid-cols-[1fr_320px] gap-6">
        ...
        <div className="bg-card border border-border rounded-2xl p-6 md:p-8">
          ...
          <div className="flex items-center gap-2 mb-8">
            {[
              ...
              <div key={s.n} className="flex items-center gap-2 flex-1">
                <div className={`h-8 w-8 shrink-0 rounded-full flex items-center justify-center text-xs font-bold transition ${
          ...
          <div className="mt-8 flex items-center justify-between">
            <button
              onClick={() => (step > 1 ? setStep(step - 1) : navigate({ to: "/clinic/login" }))}
