...
import {
  LayoutDashboard, CreditCard, LogOut, Bell,
  CheckCircle2, X, Download, Search, Filter,
  TrendingUp, Wallet, AlertCircle, Receipt,
  Clock, ChevronDown, Printer, RefreshCw
} from "lucide-react";
...
const initialPayments = [
  { id: "P001", patient: "Anita Kumar",   doctor: "Dr. Sharma", dept: "Cardiology",   time: "09:00 AM", type: "New Patient", fee: 500,  paid: true,  mode: "Online (UPI)",   ref: "UPI2024001" },
  { id: "P002", patient: "Rahul Das",     doctor: "Dr. Sharma", dept: "Cardiology",   time: "09:30 AM", type: "Follow-up",   fee: 300,  paid: true,  mode: "Cash",           ref: "CASH001" },
  { id: "P003", patient: "Meera Pal",     doctor: "Dr. Sharma", dept: "Cardiology",   time: "10:00 AM", type: "New Patient", fee: 500,  paid: false, mode: "—",              ref: "—" },
  { id: "P004", patient: "Vikram Roy",    doctor: "Dr. Sharma", dept: "Cardiology",   time: "10:30 AM", type: "New Patient", fee: 500,  paid: false, mode: "—",              ref: "—" },
  { id: "P005", patient: "Sneha Bose",    doctor: "Dr. Das",    dept: "Cardiology",   time: "11:00 AM", type: "Follow-up",   fee: 300,  paid: true,  mode: "Online (Card)",  ref: "CARD2024003" },
  { id: "P006", patient: "Arun Ghosh",    doctor: "Dr. Mehta",  dept: "Neurology",    time: "11:30 AM", type: "New Patient", fee: 700,  paid: false, mode: "—",              ref: "—" },
  { id: "P007", patient: "Priya Sen",     doctor: "Dr. Sharma", dept: "Cardiology",   time: "02:00 PM", type: "New Patient", fee: 500,  paid: true,  mode: "Online (UPI)",   ref: "UPI2024007" },
  { id: "P008", patient: "Deepak Sharma", doctor: "Dr. Das",    dept: "Cardiology",   time: "02:30 PM", type: "Follow-up",   fee: 300,  paid: false, mode: "—",              ref: "—" },
  { id: "P009", patient: "Riya Dey",      doctor: "Dr. Mehta",  dept: "Neurology",    time: "03:00 PM", type: "New Patient", fee: 700,  paid: true,  mode: "Cash",           ref: "CASH009" },
  { id: "P010", patient: "Sanjay Tiwari", doctor: "Dr. Sharma", dept: "Cardiology",   time: "05:00 PM", type: "Follow-up",   fee: 300,  paid: false, mode: "—",              ref: "—" },
];
...
export default function BillingDashboard() {
  ...
  useEffect(() => { document.title = "Billing — MedConnect"; }, []);
  ...
  const filtered = payments.filter((p) => {
    const matchQ = !q || p.patient.toLowerCase().includes(q.toLowerCase()) || p.id.toLowerCase().includes(q.toLowerCase());
  ...
  const markPaid = (id) => {
    setPayments((prev) =>
      prev.map((p) => p.id === id ? { ...p, paid: true, mode: collectMode, ref: collectRef || `REF${Date.now()}` } : p)
    );
    setCollectId(null);
    setCollectRef("");
    setCollectMode("Cash");
  ...
  return (
    <div className="min-h-screen bg-background flex">
      {/* Sidebar */}
      <aside className="hidden md:flex flex-col w-[210px] shrink-0 text-white" style={{ background: "linear-gradient(180deg, #7A0F0F 0%, #E24B4A 100%)" }}>
        <div className="p-5 border-b border-white/10">
          <div className="text-base font-bold">Med<span className="text-white/60">Connect</span></div>
          <div className="text-[10px] uppercase tracking-wider text-white/60 mt-1">Billing Portal</div>
        </div>
        <div className="p-4 border-b border-white/10 flex items-center gap-2.5">
          <div className="h-9 w-9 rounded-full bg-white/20 flex items-center justify-center text-xs font-bold">SK</div>
        ...
        <div className="mx-3 mb-3 space-y-2">
          <div className="rounded-lg bg-white/10 border border-white/15 p-3">
            ...
            <div className="text-lg font-bold font-mono mt-0.5">₹{totalCollected.toLocaleString()}</div>
          </div>
          <div className="rounded-lg bg-white/10 border border-white/15 p-3">
            <div className="text-[10px] text-white/60 uppercase">Pending</div>
            <div className="text-lg font-bold font-mono mt-0.5 text-yellow-300">₹{totalPending.toLocaleString()}</div>
          </div>
        </div>
        ...
        <header className="bg-card border-b border-border h-16 flex items-center justify-between gap-3 px-4 sm:px-6 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="relative p-2 rounded-md hover:bg-muted">
              ...
              {pending.length > 0 && <span className="absolute top-1 right-1 h-1.5 w-1.5 rounded-full bg-warning" />}
            </button>
          </div>
        </header>
            ...
            <>
              ...
              <div className="rounded-2xl p-5 sm:p-6 text-white relative overflow-hidden" style={{ background: "linear-gradient(135deg, #7A0F0F 0%, #E24B4A 100%)" }}>
                ...
                <div className="relative flex items-start justify-between flex-wrap gap-4">
                  <div>
                    <div className="text-xs uppercase tracking-wider text-white/70 font-bold">Today's billing overview</div>
                    <h2 className="mt-1 text-2xl font-bold">₹{totalCollected.toLocaleString()} collected</h2>
                    <p className="text-sm text-white/80 mt-1">₹{totalPending.toLocaleString()} still pending from {pending.length} patients</p>
                  </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                {[
                  ...
                  { label: "Online payments",   value: collected.filter(p=>p.mode.includes("Online")).length, tone: "primary",   icon: CreditCard },
                  { label: "Cash payments",     value: collected.filter(p=>p.mode==="Cash").length,           tone: "emergency", icon: Receipt },
                ].map((s) => (
                  <div key={s.label} className="rounded-xl bg-card border border-border p-4 hover:shadow-lg hover:-translate-y-0.5 transition">
                    ...
                    <div className={`mt-1 text-2xl font-bold text-${s.tone} font-mono`}>{s.value}</div>
                  </div>
                ))}
              </div>
              ...
              {pending.length > 0 && (
                <div className="rounded-xl bg-warning-soft border border-warning/20 p-5">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mb-3">
                    ...
                    <span className="text-xs text-warning font-medium">{pending.length} patients · ₹{totalPending.toLocaleString()} due</span>
                  </div>
                    ...
                    {pending.map((p) => (
                      <div key={p.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-card rounded-lg px-4 py-3 border border-warning/20">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <button
                            onClick={() => { setCollectId(p.id); setView("Collect Payment"); }}
              ...
              <div className="grid lg:grid-cols-2 gap-4">
                <div className="rounded-xl bg-card border border-border p-5">
                  ...
                  {[
                    { label: "Online — UPI",  count: collected.filter(p=>p.mode.includes("UPI")).length,  amount: collected.filter(p=>p.mode.includes("UPI")).reduce((s,p)=>s+p.fee,0) },
                    { label: "Online — Card", count: collected.filter(p=>p.mode.includes("Card")).length, amount: collected.filter(p=>p.mode.includes("Card")).reduce((s,p)=>s+p.fee,0) },
                    { label: "Cash",          count: collected.filter(p=>p.mode==="Cash").length,         amount: collected.filter(p=>p.mode==="Cash").reduce((s,p)=>s+p.fee,0) },
                  ].map((r) => (
                    <div key={r.label} className="flex items-center justify-between rounded-md bg-secondary px-4 py-3 mb-2 last:mb-0">
                      <div>
                    ...
                    return (
                      <div key={dept} className="mb-3 last:mb-0">
                        ...
                        <div className="h-2 bg-muted rounded-full overflow-hidden">
                          <div className="h-full bg-emergency rounded-full" style={{ width: `${Math.min(100,(amt/max)*100)}%` }} />
                        </div>
                      </div>
                    );
                  })}
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3 flex-wrap">
                <div className="flex-1 min-w-[200px] flex items-center gap-2 bg-input border border-border rounded-md px-3 focus-within:border-emergency">
                  ...
                  <input value={q} onChange={(e)=>setQ(e.target.value)} placeholder="Search patient or ID..." className="bg-transparent py-2 text-sm outline-none flex-1" />
                </div>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-secondary text-xs uppercase tracking-wider text-muted-foreground">
                      <tr>
                        ...
                        <th className="text-left px-5 py-3 font-medium hidden md:table-cell">Doctor</th>
                    ...
                    <tbody className="divide-y divide-border">
                      {filtered.map((p)=>(
                        <tr key={p.id} className="hover:bg-secondary/50">
                          ...
                          <td className="px-5 py-3 text-muted-foreground hidden md:table-cell">{p.doctor}</td>
                          <td className="px-5 py-3 font-mono text-xs text-muted-foreground hidden md:table-cell">{p.time}</td>
                          <td className="px-5 py-3 font-mono font-semibold">₹{p.fee}</td>
                          <td className="px-5 py-3"><PayChip paid={p.paid} mode={p.mode} /></td>
          ...
          {view === "Collect Payment" && (
            <div className="max-w-xl space-y-4">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Select a patient and mark their payment as received.</p>
              </div>

              {/* Patient picker */}
              <div className="rounded-xl bg-card border border-border p-4">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">Select patient (unpaid)</h3>
                <div className="space-y-2">
                  {pending.map((p)=>(
                    <button key={p.id} onClick={()=>setCollectId(p.id)}
                      className={`w-full text-left rounded-lg p-3 border transition ${
                        collectId===p.id ? "border-success bg-success-soft" : "border-border hover:border-success/40"
                      }`}>
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="text-sm font-medium">{p.patient}</div>
                          <div className="text-[11px] text-muted-foreground">{p.doctor} · {p.time} · {p.type}</div>
                        </div>
                        <div className="font-mono font-bold text-warning">₹{p.fee}</div>
                      </div>
                    </button>
              ...
              {collectTarget && (
                <div className="rounded-xl bg-card border border-border p-5 space-y-4">
                  <div className="rounded-lg bg-secondary p-3 text-sm">
                    <div className="flex justify-between"><span className="text-muted-foreground">Patient</span><span className="font-medium">{collectTarget.patient}</span></div>
                    <div className="flex justify-between mt-1"><span className="text-muted-foreground">Fee due</span><span className="font-mono font-bold text-warning">₹{collectTarget.fee}</span></div>
                  </div>
                  <div>
                    <label className="block text-xs text-muted-foreground mb-1.5 font-medium">Payment mode</label>
                    <div className="grid grid-cols-2 gap-2">
                      {MODES.map((m)=>(
                        <button key={m} onClick={()=>setCollectMode(m)}
                          className={`rounded-lg border p-3 text-sm font-medium transition ${
                            collectMode===m ? "border-success bg-success-soft text-success" : "border-border hover:border-success/40"
                          }`}>
                          {m}
                        </button>
                      ))}
                  ...
                  <div>
                    <label className="block text-xs text-muted-foreground mb-1.5 font-medium">Reference / Transaction ID (optional)</label>
                  ...
                  <button onClick={()=>markPaid(collectTarget.id)}
          ...
          {view === "Reports" && (
            <div className="space-y-4">
              <div className="grid lg:grid-cols-3 gap-4">
                {[
                  { label: "Today",      v: `₹${totalCollected.toLocaleString()}`, sub: `${collected.length} transactions` },
              ...
              <div className="rounded-xl bg-card border border-border p-5">
                ...
                <div className="grid sm:grid-cols-3 gap-3">
                  {["Today's report","Weekly report","Monthly report"].map((r)=>(
                    <button key={r} className="rounded-lg border border-border p-4 text-sm font-medium hover:border-emergency hover:text-emergency transition inline-flex items-center gap-2">
